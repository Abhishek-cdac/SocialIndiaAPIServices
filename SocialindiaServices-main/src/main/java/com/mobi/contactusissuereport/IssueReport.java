...
public class IssueReport extends ActionSupport implements Serializable{
	...
	public String execute() {
		...
		try {
			...
			log.logMessage("Enter into report issue", "info", IssueReport.class);
			if (Commonutility.checkempty(ivrparams)) {
				System.out.println("111111---------"+ivrparams);
...
//						}
			...
			log.logMessage("flg :" +flg, "info", IssueReport.class);
			if (flg) {
...
//						}
						...
						if (reportId != -1) {
							if (flg) {
								...
								log.logMessage("report isue size:" + userId, "info", IssueReport.class);								
							}	
...
//						serverResponse(ivrservicecode,"01","R0026",getText("R0026"),locObjRspdataJson);	
				...
				log.logMessage("Field error block", "info", IssueReport.class);
				locObjRspdataJson=new JSONObject();
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage(getText("Eex") + ex, "error", IssueReport.class);
