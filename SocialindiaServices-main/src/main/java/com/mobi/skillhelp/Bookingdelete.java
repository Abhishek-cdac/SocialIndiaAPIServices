...
public class Bookingdelete extends ActionSupport{
	...
	public String execute() {
		...
		try {
			...
			log.logMessage("Enter into Bookingdelete ", "info", Bookingdelete.class);
			if (Commonutility.checkempty(ivrparams)) {
				...
				if (ivIsJson) {
					...
					if (locObjRecvdataJson !=null) {
						...
						String locBooking = (String) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson,"booking_id");
						if (Commonutility.checkempty(locBooking) && Commonutility.toCheckisNumeric(locBooking)) {
							bookingId = Commonutility.stringToInteger(locBooking);
						} else {
							flg = false;
			...
			log.logMessage("flg :" +flg, "info", Bookingdelete.class);
			if (flg) {
				...
				flg = skillService.bookingDelete(servicebookVO);
				if (flg) {
					serverResponse(ivrservicecode,getText("status.success"),"R0031",getText("R0031"),locObjRspdataJson);
				} else {
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage(getText("Eex") + ex, "error", Bookingdelete.class);
