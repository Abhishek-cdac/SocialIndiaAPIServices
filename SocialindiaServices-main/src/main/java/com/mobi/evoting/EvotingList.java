...
import com.mobi.carpooling.CarPoolDao;
...
public class EvotingList extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		System.out.println("************mobile Evoting search List services******************");
		...
		try{
			...
			int count=0;String locVrSlQry="";
				...
				String searchField="where EMST.STATUS_FLAG=1 and usr.SOCIETY_ID='"+userMst.getSocietyId().getSocietyId()+"'";
				String minvoteSearch="";
				if(search!=null && search.length()>0){
					searchField+=" and (EMST.TITILE like('%"+search+"%') or EMST.DESCRIPTION like ('%"+search+"%')) ";
				}
				System.out.println("publishedStatus== "+publishedStatus);
				...
				if(minVote!=null && minVote.length()>0){
					if(minVote.equalsIgnoreCase("0")){
						minvoteSearch+=" where TOTAL_VOTING_STATUS ='0'";
					}else if(minVote.equalsIgnoreCase("1")){
						minvoteSearch+=" where TOTAL_VOTING_STATUS >'0'";
					}
				...
				String orderbyqry="order by EMST.EVOTING_ID desc";
				if(orderBy!=null && orderBy.length()>0){
					orderbyqry="order by EMST.TITILE "+orderBy;
				}
				
				String totcountqry="SELECT count(rid) "
						...
						+ "EMST.EVOTING_ID EVOTING_ID,EMST.TITILE TITILE,EMST.DESCRIPTION DESCRIPTION,EMST.STATUS_FLAG STATUS_FLAG,"
						+ "EMST.PUBLISH_FLAG PUBLISH_FLAG,EIMG.IMAGE_NAME IMAGE_NAME,"
						+ "IFNULL((SELECT COUNT(VOTING_STATUS)  FROM MVP_EVOTE_POOLING_TBL WHERE EVOTING_MST_ID=EMST.EVOTING_ID AND VOTING_STATUS=1 GROUP BY EVOTING_MST_ID),0) AGREE,"
						...
						+ "IFNULL((SELECT COUNT(VOTING_STATUS)  FROM MVP_EVOTE_POOLING_TBL WHERE EVOTING_MST_ID=EMST.EVOTING_ID  GROUP BY EVOTING_MST_ID),0) TOTAL_VOTING_STATUS,"
						+ "EMST.ENTRY_DATETIME entryTime "
						+ "FROM MVP_EVOTING_MST_TBL EMST LEFT JOIN MVP_EVOTING_IMAGE_TBL EIMG ON EMST.EVOTING_ID=EIMG.EVOTING_ID LEFT JOIN USR_REG_TBL USR ON EMST.ENTRY_BY=USR.USR_ID "
						+ " "+searchField+" and EMST.ENTRY_DATETIME <STR_TO_DATE('" + locTimeStamp + "','%Y-%m-%d %H:%i:%S') "+orderbyqry+")TT "+minvoteSearch;
				...
				locVrSlQry="SELECT rid,creator_emailid,mobileno,firstName,lastName,imagename,socialImage,EVOTING_ID,TITILE,DESCRIPTION,"
						+ "STATUS_FLAG,PUBLISH_FLAG,IMAGE_NAME,AGREE,DISAGREE,NEED_MEETING,NO_VOTE,TOTAL_VOTING_STATUS,entryTime,EVOTING_IMG_ID "
						+ "FROM (SELECT USR.USR_ID rid,USR.EMAIL_ID creator_emailid,USR.MOBILE_NO as mobileno,usr.FNAME firstName,usr.LNAME lastName,"
						+ "usr.IMAGE_NAME_MOBILE imagename,usr.SOCIAL_PICTURE socialImage,"
						+ "EMST.EVOTING_ID EVOTING_ID,EMST.TITILE TITILE,EMST.DESCRIPTION DESCRIPTION,EMST.STATUS_FLAG STATUS_FLAG,"
						+ "EMST.PUBLISH_FLAG PUBLISH_FLAG,EIMG.IMAGE_NAME IMAGE_NAME,"
						...
						+ "IFNULL((SELECT COUNT(VOTING_STATUS)  FROM MVP_EVOTE_POOLING_TBL WHERE EVOTING_MST_ID=EMST.EVOTING_ID AND VOTING_STATUS=4 GROUP BY EVOTING_MST_ID),0) NO_VOTE,"
						+ "IFNULL((SELECT COUNT(VOTING_STATUS)  FROM MVP_EVOTE_POOLING_TBL WHERE EVOTING_MST_ID=EMST.EVOTING_ID  GROUP BY EVOTING_MST_ID),0) TOTAL_VOTING_STATUS,"
						+ "EMST.ENTRY_DATETIME entryTime,EIMG.EVOTING_IMAGE_ID EVOTING_IMG_ID "
						+ "FROM MVP_EVOTING_MST_TBL EMST LEFT JOIN MVP_EVOTING_IMAGE_TBL EIMG ON EMST.EVOTING_ID=EIMG.EVOTING_ID LEFT JOIN USR_REG_TBL USR ON EMST.ENTRY_BY=USR.USR_ID "
						+ " "+searchField+" and EMST.ENTRY_DATETIME <STR_TO_DATE('" + locTimeStamp + "','%Y-%m-%d %H:%i:%S') "+orderbyqry+")TT "+minvoteSearch;
				System.out.println("=======locVrSlQry======="+locVrSlQry);
				evotingMstList=evotingmobhbm.getEvotingMastergListbysqlQuery(locVrSlQry,startlimit,getText("total.row"),locTimeStamp);
				...
				if(evotingMstList!=null && evotingMstList.size()>0){
					...
					for(Iterator<Object[]> it=evotingMstList.iterator();it.hasNext();) {
						...
						String externalUserImagePath = System.getenv("SOCIAL_INDIA_BASE_URL") +"/templogo/profile/mobile/"+creatorUsrId +"/";
						...
						String entrytimr=(String)objList[18];
						...
						if(evotingImage!=null){
							int evotImageId=(Integer)objList[19];
							imageObj.put("img_id", ""+evotImageId);
						String externalevotingImagePath = System.getenv("SOCIAL_INDIA_BASE_URL") +"/templogo/Evoting/mobile/"+resevotingId +"/";
						...
						int agreecount=(Integer)objList[13];
						int disagreecount=(Integer)objList[14];
						int needmeetingcount=(Integer)objList[15];
						int novotcount=(Integer)objList[16];
						int totalvotes=(Integer)objList[17];
						
						finalJsonarr.put("agree",""+agreecount);
						finalJsonarr.put("disagree",""+disagreecount);
						finalJsonarr.put("need_meeting",""+needmeetingcount);
						finalJsonarr.put("no_vote",""+novotcount);
						finalJsonarr.put("total_votes",""+totalvotes);
