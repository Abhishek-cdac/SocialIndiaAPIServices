...
public class CarPoolDelete extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		System.out.println("************mobile CarPool Delete services******************");
		...
		try{
			...
			if(userMst!=null){
				...
				if(carpoolId!=null && carpoolId.length()>0){
				String sql="update CarPoolingTblVO set status=0 where poolId="+carpoolId+" and entryBy.userId="+rid;
				boolean carPoolingId=carPoolinghbm.runupdateQueryCarPoolingTable(sql);
				...
				if(carPoolingId){
					serverResponse(servicecode,"00","R0246",mobiCommon.getMsg("R0246"),locObjRspdataJson);
				}else{
					locObjRspdataJson=new JSONObject();
