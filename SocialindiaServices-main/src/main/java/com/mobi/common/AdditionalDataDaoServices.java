	...
	public String formAdditionalDataForFeedTbl(FeedsTblVO feedTblObj) {
		...
		try{
			...
			JsonSimplepackDao jsonDataPack = new JsonSimplepackDaoService();
			 ...
			 String imagePathWeb = System.getenv("SOCIAL_INDIA_BASE_URL")  + rb.getString("external.templogo") + rb.getString("external.uploadfile.feed.img.webpath");
				String imagePathMobi = System.getenv("SOCIAL_INDIA_BASE_URL")  + rb.getString("external.templogo") +  rb.getString("external.uploadfile.feed.img.mobilepath");
				String videoPath = System.getenv("SOCIAL_INDIA_BASE_URL")  + rb.getString("external.templogo") +  rb.getString("external.uploadfile.feed.videopath");
				String videoPathThumb = System.getenv("SOCIAL_INDIA_BASE_URL")  + rb.getString("external.templogo") +  rb.getString("external.uploadfile.feed.video.thumbpath");
				String profileimgPath = System.getenv("SOCIAL_INDIA_BASE_URL") + rb.getString("external.templogo") + rb.getString("external.view.profile.mobilepath");
		...
		try{
			...
			for(Iterator<Object[]> it=eventListObj.iterator();it.hasNext();) {
				...
				if((String)objList[9]!=null){
					String externalUserImagePath = System.getenv("SOCIAL_INDIA_BASE_URL") +"/templogo/profile/mobile/"+eventObj.getIvrBnEVENT_CRT_USR_ID()+"/"+(String)objList[9];
				...
				if((String)objList[16]!=null){
					...
					String eventpath=System.getenv("SOCIAL_INDIA_BASE_URL") +"/templogo/"+rb.getString("external.inner.eventfldr")+"/mobile/"+eventId+"/"+(String)objList[16];
