...
public class MerchantKeySearchList extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		try{
			...
			int count=0;String locVrSlQry="";
				...
				String searchField="where mrchntActSts=1";
				if(keyForSearch!=null && keyForSearch.length()>0){
					searchField+=" and  keyForSearch like ('%"+keyForSearch+"%')";
				}
				
				
							
				locVrSlQry="from MerchantTblVO "+ searchField +"  group by keyForSearch";
				merchantTblList=merchanthbm.getAllMerchantCategoryList(locVrSlQry,getText("total.row"));
				System.out.println("locVrSlQry--------------"+locVrSlQry);
				System.out.println("=======merchantCategoryList size======="+merchantTblList.size());
				JSONObject finalJsonarr=new JSONObject();
				locObjRspdataJson=new JSONObject();
				JSONArray jArray =new JSONArray();
				if(merchantTblList!=null && merchantTblList.size()>0){
					MerchantTblVO merchantTblObj;
					String formatToString="yyyy-MM-dd HH:mm:ss";
					String destPath ="";
					for(Iterator<MerchantTblVO> it=merchantTblList.iterator();it.hasNext();){
						merchantTblObj=it.next();
						finalJsonarr=new JSONObject();
						
						finalJsonarr.put("key_for_search", merchantTblObj.getKeyForSearch());
						//finalJsonarr.put("merchant_id", merchantTblObj.getMrchntId().toString());
						jArray.put(finalJsonarr);
					}
					
				locObjRspdataJson.put("merchantdetails", jArray);
			//	locObjRspdataJson.put("totalrecords",merchantTblList.size());
