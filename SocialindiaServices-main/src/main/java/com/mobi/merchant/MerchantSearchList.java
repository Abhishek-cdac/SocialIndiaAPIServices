			...
			int count=0;String locVrSlQry="";
				...
				*/
				...
				for(Iterator<Object[]> it=contactListObj.iterator();it.hasNext();) {
					...
					String logPath="";
					if(objList[3]!=null){
						logPath=System.getenv("SOCIAL_INDIA_BASE_URL")  +"/templogo/"+getText("external.inner.marchantmstfldr")+getText("external.inner.mobilepath")+merchantId+"/"+objList[3];
					}
					...
					if(offerImages!=null){
						...
						for (int i=0;i<offerImgArray.length;i++){
							String offerImg = System.getenv("SOCIAL_INDIA_BASE_URL")  +"/externalPath/offers/mobile/"+offerImgArray[i];
							imageobj=new JSONObject();
							imageobj.put("img_id", "");
