...
public class MakeRSVP extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		System.out.println("************mobile MakeRSVP services******************");
		...
		try{
			...
			if (ivIsJson) {		
				...
				String rsvpFlag = (String) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson, "rsvp_flow");
			...
			int updstatus=-1;
			if(count>0){
			String query="update EventDispTblVO set rsvpFlag='"+rsvpFlag+"' where ivrBnUAMMSTtbvoobj.userId='"+rid+"' and ivrBnEVENT_ID.ivrBnEVENT_ID='"+eventId+"'";
			...
			System.out.println("updstatus-----------"+updstatus);
				...
				eventdispobj.setContributeAmount(0);
				eventdispobj.setRsvpFlag(Integer.parseInt(rsvpFlag));
