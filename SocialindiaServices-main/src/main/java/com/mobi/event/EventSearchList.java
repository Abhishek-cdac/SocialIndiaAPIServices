			...
			//if(eventListObj!=null &&  eventListObj1!=null  && (eventListObj.size()>0 || eventListObj1.size()>0)){
				...
				for(Iterator<Object[]> it=eventListObj.iterator();it.hasNext();) {
					...
					if((String)objList[9]!=null){
						String externalUserImagePath = System.getenv("SOCIAL_INDIA_BASE_URL") +"/templogo/profile/mobile/"+rid+"/"+(String)objList[9];
					...
					if((String)objList[16]!=null){
						...
						String eventpath=System.getenv("SOCIAL_INDIA_BASE_URL") +"/templogo/"+getText("external.inner.eventfldr")+"/mobile/"+eventId+"/"+(String)objList[16];
