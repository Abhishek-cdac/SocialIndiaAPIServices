...
import com.mobi.feed.FeedCommentPost;
...
public class SkillNotification extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			log.logMessage("Enter into SkillNotification ", "info",
					FeedCommentPost.class);
			...
			if (flg) {
				...
				if (to_usr_id != rid) {
					...
					UserMasterTblVo entryBy = uam.getregistertable(rid);

					NotificationDao notificationHbm = new NotificationDaoServices();
					notificationHbm.insertnewNotificationDetails(toUserMasterTblVo, entryBy.getFirstName() + " shows interest in your skill.", 11, 0, 0, entryBy, null);
				}
				// end
				serverResponse(ivrservicecode, getText("status.success"),"R0040", "interest for skill notification sucessfull", locObjRspdataJson);
			} else {
				locObjRspdataJson = new JSONObject();
				locObjRspdataJson.put("fielderror", Commonutility
						.removeSPtag(locErrorvalStrBuil.toString()));
				serverResponse(ivrservicecode,
