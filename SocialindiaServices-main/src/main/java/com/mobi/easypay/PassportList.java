...
public class PassportList extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String searchQury = "";
			int totCnt = 0;
			log.logMessage("Enter into PassportList ", "info", PassportList.class);
			if (Commonutility.checkempty(ivrparams)) {
				ivrparams = EncDecrypt.decrypt(ivrparams);
				log.logMessage("PassportList ivrparams :" + ivrparams, "info", PassportList.class);
			...
			log.logMessage("flg :" +flg, "info", PassportList.class);
			if (flg) {
				...
				if(flg){
					...
					if (flg) {
						...
						List<PassportTblVo> listObj = new ArrayList<PassportTblVo>();
						listObj = passportDaoObj.passportListDetail(passportObj);
						JSONArray jArray =new JSONArray();
						if (listObj != null && listObj.size() > 0) {
							totCnt = listObj.size();
							jArray = passportDaoObj.passportDetailsPack(listObj);
							String contQuery = "select count(passId) from  PassportTblVo where status <>'0'" + searchQury;
							log.logMessage("PassportList contQuery:" + contQuery, "info", PassportList.class);
							//totCnt = commonServ.getTotalCountQuery(contQuery);
							System.out.println("passport list---------->> " + jArray.toString());
							locObjRspdataJson.put("passport_details", jArray);
							locObjRspdataJson.put("totalrecords", totCnt);
							if (flg) {
								serverResponse(ivrservicecode,getText("status.success"),"R0217",mobiCommon.getMsg("R0217"),locObjRspdataJson);
							} else {
								locObjRspdataJson = new JSONObject();
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage(getText("Eex") + ex, "error", PassportList.class);
