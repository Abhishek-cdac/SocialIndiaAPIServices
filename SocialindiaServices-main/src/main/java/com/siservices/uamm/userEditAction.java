...
public class userEditAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			if (ivIsJson) {
				System.out.println("Step 1 : User Edit Action Called.");
				...
				finalJson.put("noofFlats", userData.getNoofflats());				
				if(userData.getFlatNo()==null){
					finalJson.put("flatNo", 0);	
				}else{
					finalJson.put("flatNo", userData.getFlatNo());	
				}if(userData.getNoofblocks()!=null){
					finalJson.put("noofwings", userData.getNoofblocks());	
				}else{
					finalJson.put("noofwings", "");	
				}
				
				if(userData.getNumOfLogins()!=null){
					finalJson.put("numOfLogins", userData.getNumOfLogins());	
				}else{
					finalJson.put("numOfLogins", "");	
				}
...
//				System.out.println("userData.getPstlId() >>>>>>>>>>>>>>>>>>>>>>> "+userData.getPstlId());
				
				if(userData.getPstlId()!=null){
//					finalJson.put("pincode", userData.getPstlId().getPstlId());
//					finalJson.put("pincodeName", userData.getPstlId().getPstlCode());
...
//					finalJson.put("pincodeName", "");
				...
				if(userData.getMembersInFamily()==0){
					finalJson.put("memberofFamily",0);
				}else{
				...
				System.out.println("userFamilyList.size()   "+userFamilyList.size());
				if( userFamilyList!=null && userFamilyList.size()>0){
					MvpFamilymbrTbl userObj;
				for (Iterator<MvpFamilymbrTbl> it = userFamilyList
						.iterator(); it.hasNext();) {
					userObj = it.next();
					finalJsonarr1 = new JSONObject();
					finalJsonarr1.put("childmobile", userObj.getFmbrPhNo());
					finalJsonarr1.put("childemail", userObj.getFmbrEmail());
					finalJsonarr1.put("childname", userObj.getFmbrName());
					finalJsonarr1.put("fmbrisd", userObj.getFmbrIsdCode());
				...
				System.out.println("Step 2: user Edit Action End.");
			...
			ex.printStackTrace();
			System.out.println("Exception found userEditAction.class : " + ex);
			log.logMessage("Service : userEditAction error occurred : " + ex, "error",userEditAction.class);
			json = new JSONObject();
