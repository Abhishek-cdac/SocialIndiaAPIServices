...
public class siTownshipMasterTblAction extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			if (ivIsJson) {
				...
				locVrSlQry="select  townshipId, townshipName from TownshipMstTbl ";
				...
				locObjdataJson=new JSONObject();
				serverResponse("SI0035","1","E0001","Request values not not correct format",locObjdataJson);
			}}else{
				locObjdataJson=new JSONObject();
				log.logMessage("Service code : SI2002, Request values are not correct format", "info", siTownshipMasterTblAction.class);
				serverResponse("SI0035","1","E0001","Request values are not correct format",locObjdataJson);
			}
		} catch (Exception e) {
			System.out.println("Exception found in siTownshipMasterTblAction.action execute() Method : "+ e);
			locObjdataJson=new JSONObject();			
