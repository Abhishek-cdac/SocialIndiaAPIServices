...
public class siSocietyMgmtView extends ActionSupport{
	...
	public String execute(){
		...
		try{
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				jsonFinalObj.put("colorcode", societyMst.getColourCode());
...
//				jsonFinalObj.put("postalcode", societyMst.getTownshipId().getPstlId().getPstlCode());
				...
				jsonFinalObj.put("gstin", societyMst.getGstin());
				jsonFinalObj.put("hsn",societyMst.getHsn());
				
				if(societyMst.getNoOfBlocksWings()==null){
					jsonFinalObj.put("noofblockswings", "");
				}else{
				jsonFinalObj.put("noofblockswings", societyMst.getNoOfBlocksWings());
				}
				if(societyMst.getRegisterNo()==null){
					jsonFinalObj.put("registerno", "");
				}else{
				...
				if(societyAcc!=null){
					...
					jsonFinalObj.put("bankname", societyAcc.getBankName());
					}else{
				...
				jsonFinalObj.put("icoimage", Commonutility.toCheckNullEmpty(societyMst.getIcoImage()));
				jsonFinalObj.put("logoimage", Commonutility.toCheckNullEmpty(societyMst.getLogoImage()));
			...
			log.logMessage("Service code : ivrservicecode, Sorry, an unhandled error occurred", "info", siSocietyMgmtView.class);
			try {
