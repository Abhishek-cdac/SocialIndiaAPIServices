...
public class TownshipList extends ActionSupport{
	...
	public String execute(){
		...
		try{
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				lsvSlQry="select townshipId,townshipName from TownshipMstTbl where status='1'";				
				...
				serverResponse(ivrservicecode,"0","0000","success",locObjRspdataJson);
					...
					locObjRspdataJson=new JSONObject();
					log.logMessage("Service code : SI6432, Request values are not correct format", "info", TownshipList.class);
					serverResponse("SI6432","1","E0001","Request values are not correct format",locObjRspdataJson);
				}	
			}else{
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI6432, Request values are empty", "info", TownshipList.class);
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : SI6432, Sorry, an unhandled error occurred", "error", TownshipList.class);
