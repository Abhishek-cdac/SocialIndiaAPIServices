...
public class ResidentExcelEngine extends Thread{
	...
	public ResidentExcelEngine(){
		System.out.println(" default ResidentExcelEngine (): [ResidentExcelEngine] Thread.");
	}
	
	public ResidentExcelEngine(String uploadFldrname, String pBkupfladrName, String pgrpName, String pUpldFilepath,int pCurntUsrid, String pFileExtnce,int pFileid){
		System.out.println("ResidentExcelEngine (): [ResidentExcelEngine] Thread.");
	...
	public void run(){
		System.out.println("Step 1 : [ResidentExcelEngine] Thread Started.");
		String rsss=toInsertResident(ivrGrpName,ivrUploadFilePath,ivrCurnUsrid,1,6,ivrExtnce,ivrFileid);	
		if(rsss!=null && rsss.equalsIgnoreCase("done") && this.isAlive()){	
	...
	public String toInsertResident(String pGrpName,String locuplfilepath,int ivrEntryByusrid, int startrow, int columnlength, String pFileXtnce,int pFileid){
		...
		try {
			...
			System.out.println("Step 2 : [ResidentExcelEngine] toInsertResident Block Enter. ResidentUtility.readExcel() Start");
			...
			System.out.println("Step 3 : [ResidentExcelEngine] toInsertResident ResidentUtility.readExcel() End." + locvrFnrst);
			...
			if((locvrFnrst!=null && !locvrFnrst.equalsIgnoreCase("null") && !locvrFnrst.equalsIgnoreCase(""))) {
				...
				String locerr_id="";
				boolean flg = true;
				log.logMessage("ResidentExcelEngine Thread via insert will start.", "info", ResidentExcelEngine.class);
				//i value of row
				//j value of column
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					try {
						...
						System.out.println("Exception Found in ResidentExcelEngine toInsertResident() : "+ex);
				...
				log.logMessage("Empty File Uploaded", "info", ResidentExcelEngine.class);
			}
			if(this.isAlive()){
				this.stop();
			}		
			...
			System.out.println("Step -1 : Exception Found in ResidentExcelEngine : "+ee);
			log.logMessage("Exception : "+ee, "error", ResidentExcelEngine.class);
			return "done";
	...
	public static void main(String argg[]){
		//ResidentExcelEngine nn=new ResidentExcelEngine("E:/FileMoveTest/", "E:/backup/","Resident","E:/FileMoveTest/Testexcel.xls",1,"xls");
		//nn.setName("RPKARANThread");		
		//nn.start();
		//boolean tt=nn.isAlive();
		//System.out.println(tt+" : "+nn.getName());
	}
}
