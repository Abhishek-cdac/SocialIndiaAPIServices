...
public class Knownuslistviewall extends ActionSupport {
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				String errocheck=(String) Commonutility.toHasChkJsonRtnValObj(locObjRspdataJson, "CatchBlock");// if Catch block found error occurred in select
					...
					//	AuditTrial.toWriteAudit(getText("WRKTYPAD014"), "WRKTYPAD014", ivrCurrntusrid);
						serverResponse("SI38000","0","E38000",getText("knownus.selectall.error"),locObjRspdataJson);
					}else{
						//AuditTrial.toWriteAudit(getText("WRKTYPAD013"), "WRKTYPAD013", ivrCurrntusrid);
						serverResponse("SI38000","0","S38000",getText("knownus.selectall.success"),locObjRspdataJson);					
					}
				
				}else{
					locObjRspdataJson=new JSONObject();
					log.logMessage("Service code : SI38000, "+getText("request.format.notmach"), "info", Knownuslistviewall.class);
					serverResponse("SI38000","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);

				}	
			}else{
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI38000, "+getText("request.values.empty"), "info", Knownuslistviewall.class);
				serverResponse("SI38000","1","ER0001",getText("request.values.empty"),locObjRspdataJson);

			}	
		}catch(Exception e){
			System.out.println("Exception found .class execute() Method : "+e);
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : SI38000, "+getText("catch.error"), "error", Knownuslistviewall.class);
			serverResponse("SI38000","2","ER0002",getText("catch.error"),locObjRspdataJson);
		}finally{
	...
	public static JSONObject toTitlelistSelectAll(JSONObject pDataJson, Session pSession) {
		...
		KnownusTblVO lbrDtlVoObj=null;
		int count=0,countFilter=0, startrowfrom=1, totalNorow=10;		
		try {
			log=new Log();	lbrDtlVoObj=new KnownusTblVO();			
			System.out.println("Step 1 : knownus Type  select all block enter");
			log.logMessage("Step 1 : knownus Type  select all block enter", "info", Knownuslistviewall.class);			
			...
			if(locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))){// for web 
				...
				{
					locvrEdu_cntQry="select count(iVOKNOWN_US_ID) from KnownusTblVO where iVOACT_STS in(0,1)";
				}
				else
				{
					locvrEdu_cntQry="select count(iVOKNOWN_US_ID) from KnownusTblVO where iVOACT_STS in(0,1) AND (" + "iVOKNOWNUS like ('%" + loctocheNull+ "%')  " 
			                                + ")";
				}
				System.out.println("Step 2 : Count / Filter Count block enter SlQry : "+locvrEdu_cntQry);
				log.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+locvrEdu_cntQry, "info", Knownuslistviewall.class);
				...
				countFilter=IdcardDaoObj.getTotalFilter(locvrEdu_cntQry);
				 ...
				 log.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", Knownuslistviewall.class);
			}
			...
			if(loctocheNull!=null && !loctocheNull.equalsIgnoreCase("null") && !loctocheNull.equalsIgnoreCase("")){
				  globalsearch = " AND (" + "iVOACT_STS like ('%" + loctocheNull+ "%') or " 
			                                + "iVOKNOWNUS like ('%" + loctocheNull + "%'))";
				  loc_slQry="from KnownusTblVO  where iVOACT_STS in(0,1) " +globalsearch;
			}else{
				loc_slQry="from KnownusTblVO  where iVOACT_STS in(0,1)";	
			}				
			//loc_slQry="from LaborDetailsTblVO  where ivrBnLBR_STS="+locvrLBR_STS+"";	
			System.out.println("Step 3 : knownus Type  Details Query : "+loc_slQry);
			log.logMessage("Step 3 : knownus Type  Details Query : "+loc_slQry, "info", Knownuslistviewall.class);
			...
			while ( locObjLbrlst_itr.hasNext() ) {
				...
				lbrDtlVoObj = (KnownusTblVO) locObjLbrlst_itr.next();								
				locInrJSONObj.put("knownus_id",lbrDtlVoObj.getiVOKNOWN_US_ID());	
				locInrJSONObj.put("knownus_name",lbrDtlVoObj.getiVOKNOWNUS());
				locInrJSONObj.put("knownus_status",lbrDtlVoObj.getiVOACT_STS());									
			...
			locFinalRTNObj.put("knownusdetails", locLBRJSONAryobj);			
			System.out.println("locFinalRTNObj : "+locFinalRTNObj);
			System.out.println("Step 6 : Select category detail block end.");
			log.logMessage("Step 4: Select category detail block end.", "info", Knownuslistviewall.class);
			return locFinalRTNObj;
		} catch (Exception e) {
			try{
			System.out.println("Exception in toCMPYSelectAll() : "+e);
			log.logMessage("Step -1 : category select all block Exception : "+e, "error", Knownuslistviewall.class);	
			locFinalRTNObj=new JSONObject();
			locFinalRTNObj.put("InitCount", count);
			locFinalRTNObj.put("countAfterFilter", countFilter);
			locFinalRTNObj.put("knownusdetails", "");
