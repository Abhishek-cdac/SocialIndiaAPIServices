...
public class PaygateuploadFile extends ActionSupport{
	/**
	 * 
	 */
	...
	public String execute(){
		System.out.println("Step 1: --------------------------- Enter paygate Services ");
		...
		try{
			...
			if (ivrparams != null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length() > 0) {
				...
				if (ivIsJson) {	
					...
					String imageDetail=(String) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson, "payFile");
					String fileName=(String) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson, "payFileName");	
					...
					if (limgName.length() > 0) {
					 ...
					 String locupFldrPath=getText("external.fldr")+getText("external.paygateuplfldr");//file upload path
					//backupfile path
					// String locuplbackupPath=getText("external.fldr")+getText("external.backupfldr");
						if((imageDetail!=null && !imageDetail.equalsIgnoreCase("null") && !imageDetail.equalsIgnoreCase("")) && (imageDetail!=null && !imageDetail.equalsIgnoreCase("") && !imageDetail.equalsIgnoreCase("null"))){
							...
							String qryCheck="select ivrBnCHECK_SUM from PaygatesetmtfileTblVo where ivrBnCHECK_SUM='"+checksum+"'";
					        ...
					        paygatedata.setIvrBnCHECK_SUM(checksum);
					        paygatedata.setIvrBnENTRY_BY(entrydet);
					        paygatedata.setIvrBnFLIENAME(file_name);
					        paygatedata.setIvrBnExSTATUS(0);
					        paygatedata.setIvrBnSTATUS(1);
					        paygatedata.setIvrBnENTRY_DATETIME(Commonutility.getCurrentDateTime("yyyy-MM-dd HH:mm:ss"));
					        int rowid=reconile.toInsertPaygateData(paygatedata);
					        System.out.println("Step 3: ------rowid ---------- "+rowid);
					        if(rowid>0){
					        	CyberPaygateCall cybercall=new CyberPaygateCall("paygate");
					        	cybercall.start();
					        }
							}
							...
							locObjRspdataJson=new JSONObject();
							log.logMessage("Service code : SI40000,"+getText("request.format.notmach")+"", "info", PaygateuploadFile.class);
				...
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI6406,"+mobiCommon.getMsg("R0003")+"", "info", PaygateuploadFile.class);
				serverResponse("SI40000","01","R0003",mobiCommon.getMsg("R0003"),locObjRspdataJson);
			}
			
		}catch (Exception ex){
			log.logMessage("Service code : SI40000,"+mobiCommon.getMsg("R0087")+"", "error", PaygateuploadFile.class);
