...
public class CyberPlatMgmtTable extends ActionSupport {
	  /**
	   *sdsd.
	   */
	  ...
	  public String execute() {
	    ...
	    try {
	    	  ...
	    	  if (ivIsJson) {
	    		...
	    		if(ivrservicecode!=null && !ivrservicecode.equalsIgnoreCase("null") && !ivrservicecode.equalsIgnoreCase("")){
	    			...
	    			if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
	    				AuditTrial.toWriteAudit(getText("CYBER0002"), "CYBER0002", ivrCurrntusrid);
	    				serverResponse("SI8006","0","E8006",getText("cyberplate.selectall.error"),locObjRspdataJson);
	    			}else{
	    				AuditTrial.toWriteAudit(getText("CYBER0001"), "CYBER0001", ivrCurrntusrid);
	    				serverResponse("SI8006","0","S8006",getText("cyberplate.selectall.success"),locObjRspdataJson);					
	    			}
	    		}else {
	    			locObjRspdataJson=new JSONObject();
	    	    	logWrite.logMessage("Service code : SI8006,"+getText("request.values.empty")+"", "info", CyberPlatMgmtTable.class);
	    			serverResponse("SI8006","1","ER0001",getText("request.values.empty"),locObjRspdataJson);
	    	    }			    	   
	    	  }else {
	          locObjRspdataJson=new JSONObject();
	          logWrite.logMessage("Service code : SI8006,"+getText("request.format.notmach")+"", "info", CyberPlatMgmtTable.class);
	      ...
	      locObjRspdataJson=new JSONObject();
	      logWrite.logMessage("Service code : SI8006, Sorry, an unhandled error occurred : "+expObj, "error", CyberPlatMgmtTable.class);
	  ...
	  private JSONObject tocyberplat(JSONObject praDatajson, Session praSession){	
		...
		CyberplatsetmtfileTblVo cyberplate = null;
	    ...
	    try {
	    	...
	    	System.out.println("Step 1 : Select CyberPlatMgmtTable process start.");
	    	logWrite.logMessage("Step 1 : Select CyberPlatMgmtTable process start.", "info", CyberPlatMgmtTable.class);
			...
			int sSoctyId=(Integer) Commonutility.toHasChkJsonRtnValObj(praDatajson, "sSoctyId");
			...
			lvSlqry = "from CyberplatsetmtfileTblVo  where ivrBnSTATUS=" + locvrEventSTS + " " + glbSearch;	
			lvrevntcountqry = "select count(*) from CyberplatsetmtfileTblVo   where  ivrBnSTATUS=" + locvrEventSTS+ " " + glbSearch;	
			ReconcileDao reconDobj=new ReconcileDaoService();
			count = reconDobj.getInitTotal(lvrevntcountqry);
			countFilter = count;
	
			System.out.println("Step 3 : CyberPlatMgmtTable Query : "+lvSlqry);
			logWrite.logMessage("Step 3 : CyberPlatMgmtTable  Query : "+lvSlqry, "info", CyberPlatMgmtTable.class);
			...
			while ( lvrObjeventlstitr.hasNext() ) {
				...
				cyberplate = (CyberplatsetmtfileTblVo) lvrObjeventlstitr.next();
				lvrInrJSONObj.put("cyberId", Commonutility.toCheckNullEmpty(cyberplate.getIvrBnCBPLT_FILE_ID()));
				lvrInrJSONObj.put("filename", Commonutility.toCheckNullEmpty(cyberplate.getIvrBnFLIENAME()));
				if(cyberplate.getIvrBnENTRY_BY().getSocietyId()!=null){
				lvrInrJSONObj.put("townshipname", Commonutility.toCheckNullEmpty(cyberplate.getIvrBnENTRY_BY().getSocietyId().getTownshipId().getTownshipName()));
				lvrInrJSONObj.put("societyname", Commonutility.toCheckNullEmpty(cyberplate.getIvrBnENTRY_BY().getSocietyId().getSocietyName()));
				}else{
					lvrInrJSONObj.put("townshipname", "");
					lvrInrJSONObj.put("societyname", "");
				}
				lvrInrJSONObj.put("entrydate", Commonutility.toCheckNullEmpty(locCommonObj.getDateobjtoFomatDateStr(cyberplate.getIvrBnENTRY_DATETIME(), "yyyy-MM-dd hh:mm:ss")));
				lvrInrJSONObj.put("downloadfilePath", Commonutility.toCheckNullEmpty(rb.getString("external.fldr")+rb.getString("external.cyberplateuplfldr")+cyberplate.getIvrBnFLIENAME()));
			...
			lvrRtnjsonobj.put("cyberplatedetails", lvrEventjsonaryobj);
			System.out.println("Step 4 : Select CyberPlatMgmtTable process End");
			logWrite.logMessage("Step 4 : Select CyberPlatMgmtTable process End", "info", CyberPlatMgmtTable.class);
	    return lvrRtnjsonobj;
	    }catch(Exception expObj) {
	    	try {
				System.out.println("Exception in CyberPlatMgmtTable() : "+expObj);
				logWrite.logMessage("Step -1 : CyberPlatMgmtTable select all block Exception : "+expObj, "error", CyberPlatMgmtTable.class);	
				lvrRtnjsonobj=new JSONObject();
				lvrRtnjsonobj.put("InitCount", count);
				lvrRtnjsonobj.put("countAfterFilter", countFilter);
				lvrRtnjsonobj.put("cyberplatedetails", "");
