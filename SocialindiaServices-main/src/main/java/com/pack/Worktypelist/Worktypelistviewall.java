...
public class Worktypelistviewall extends ActionSupport {
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				String errocheck=(String) Commonutility.toHasChkJsonRtnValObj(locObjRspdataJson, "CatchBlock");// if Catch block found error occurred in select
					if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
						...
						serverResponse("SI20000","0","E20000",getText("worktype.selectall.error"),locObjRspdataJson);
					}else{
						AuditTrial.toWriteAudit(getText("WRKTYPAD013"), "WRKTYPAD013", ivrCurrntusrid);
						serverResponse("SI20000","0","S20000",getText("worktype.selectall.success"),locObjRspdataJson);					
					}
				
				}else{
					locObjRspdataJson=new JSONObject();
					log.logMessage("Service code : SI20000, "+getText("request.format.notmach"), "info", Worktypelistviewall.class);
					serverResponse("SI20000","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);

				}	
			}else{
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI20000, "+getText("request.values.empty"), "info", Worktypelistviewall.class);
				serverResponse("SI20000","1","ER0001",getText("request.values.empty"),locObjRspdataJson);

			}	
		}catch(Exception e){
			System.out.println("Exception found .class execute() Method : "+e);
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : SI20000, "+getText("catch.error"), "error", Worktypelistviewall.class);
			serverResponse("SI20000","2","ER0002",getText("catch.error"),locObjRspdataJson);
		}finally{
	...
	public static JSONObject toWorktypelistSelectAll(JSONObject pDataJson, Session pSession) {
		...
		LaborWrkTypMasterTblVO lbrDtlVoObj=null;
		int count=0,countFilter=0, startrowfrom=1, totalNorow=10;		
		try {
			log=new Log();	lbrDtlVoObj=new LaborWrkTypMasterTblVO();			
			System.out.println("Step 1 : Worktype Type  select all block enter");
			log.logMessage("Step 1 : Worktype Type  select all block enter", "info", Worktypelistviewall.class);			
			...
			if(locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))){// for web 
				...
				{
					locvrEdu_cntQry="select count(wrktypId) from LaborWrkTypMasterTblVO where status in(0,1)";
				}
				else
				{
					locvrEdu_cntQry="select count(wrktypId) from LaborWrkTypMasterTblVO where status in(0,1) AND (" + "IVOlbrWORK_TYPE like ('%" + loctocheNull+ "%')  " 
			                                + ")";
				}
				System.out.println("Step 2 : Count / Filter Count block enter SlQry : "+locvrEdu_cntQry);
				log.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+locvrEdu_cntQry, "info", Worktypelistviewall.class);
				...
				countFilter=IdcardDaoObj.getTotalFilter(locvrEdu_cntQry);
				 ...
				 log.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", Worktypelistviewall.class);
			}
				...
				String lvrOrdby = "order by wrktypId desc";
			if(loctocheNull!=null && !loctocheNull.equalsIgnoreCase("null") && !loctocheNull.equalsIgnoreCase("")){
				  globalsearch = " AND (" + "status like ('%" + loctocheNull+ "%') or " 
			                                + "IVOlbrWORK_TYPE like ('%" + loctocheNull + "%'))";
				  loc_slQry="from LaborWrkTypMasterTblVO  where status in(0,1) " +globalsearch +" "+lvrOrdby;
			}else{
				loc_slQry="from LaborWrkTypMasterTblVO  where status in(0,1) "+lvrOrdby;	
			}				
			//loc_slQry="from LaborDetailsTblVO  where ivrBnLBR_STS="+locvrLBR_STS+"";	
			System.out.println("Step 3 : Worktype Type  Details Query : "+loc_slQry);
			log.logMessage("Step 3 : Worktype Type  Details Query : "+loc_slQry, "info", Worktypelistviewall.class);
			...
			while ( locObjLbrlst_itr.hasNext() ) {
				...
				lbrDtlVoObj = (LaborWrkTypMasterTblVO) locObjLbrlst_itr.next();								
				locInrJSONObj.put("worktype_id",lbrDtlVoObj.getWrktypId());	
				locInrJSONObj.put("worktype_name",lbrDtlVoObj.getIVOlbrWORK_TYPE());
				locInrJSONObj.put("worktype_status",lbrDtlVoObj.getStatus());									
			...
			locFinalRTNObj.put("worktypedetails", locLBRJSONAryobj);			
			System.out.println("locFinalRTNObj : "+locFinalRTNObj);
			System.out.println("Step 6 : Select category detail block end.");
			log.logMessage("Step 4: Select category detail block end.", "info", Worktypelistviewall.class);
			return locFinalRTNObj;
		} catch (Exception e) {
			try{
			System.out.println("Exception in toCMPYSelectAll() : "+e);
			log.logMessage("Step -1 : category select all block Exception : "+e, "error", Worktypelistviewall.class);	
			locFinalRTNObj=new JSONObject();
			locFinalRTNObj.put("InitCount", count);
			locFinalRTNObj.put("countAfterFilter", countFilter);
			locFinalRTNObj.put("worktypedetails", "");
