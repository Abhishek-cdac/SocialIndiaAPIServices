...
public class GetProfileDetail extends ActionSupport{
	...
	public String execute() {
		...
		try {
			...
			Date timeStamp = null;
			String locRid = null;
			String locUserid = null;
			log.logMessage("Enter into GetProfileDetail ", "info", GetProfileDetail.class);
			if (Commonutility.checkempty(ivrparams)) {
				ivrparams = EncDecrypt.decrypt(ivrparams);
				log.logMessage("GetProfileDetail ivrparams :" + ivrparams, "info", GetProfileDetail.class);
				...
				if (ivIsJson) {
					...
					if (locObjRecvdataJson !=null) {
						...
						locUserid = (String) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson,"usr_id");
					}
					...
					if (locObjRecvdataJson !=null) {
						...
						if (Commonutility.checkempty(locUserid) && Commonutility.toCheckisNumeric(locUserid)) {
							userid = Commonutility.stringToInteger(locUserid);
							if (userid !=0 ) {
			...
			log.logMessage("flg :" +flg, "info", GetProfileDetail.class);
			if (flg) {
				...
				if(flg){
					...
					if (flg) {
							String externalProfileImg = "";
							UserMasterTblVo userMastInfo = new UserMasterTblVo();
							FlatMasterTblVO flatMastInfo = new FlatMasterTblVO();
							userMastInfo = profile.getUserInfo(userid);
							System.out.println("userMastInfo result--------->> " + userMastInfo);
							if (userMastInfo != null) {
								locObjRspdataJson.put("profile_name", userMastInfo.getFirstName());
								locObjRspdataJson.put("profile_id", ""+userid);
								if (userMastInfo.getImageNameMobile() != "" && userMastInfo.getImageNameMobile() != null
										&& userMastInfo.getImageNameMobile() != "null") {
									externalProfileImg = System.getenv("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/mobile/"+userid+"/"+userMastInfo.getImageNameMobile();
								} else {
									externalProfileImg = userMastInfo.getSocialProfileUrl();
								}
								locObjRspdataJson.put("profile_image", externalProfileImg);
							} else {
								flg = false;
							}
							flatMastInfo = profile.getFlatUserInfo(userid);
							if (flatMastInfo != null) {
								locObjRspdataJson.put("block_no", flatMastInfo.getWingsname());
								locObjRspdataJson.put("flat_no", flatMastInfo.getFlatno());
							} else {
								flg = false;
							}
							if (flg) {
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage(getText("Eex") + ex, "error", GetProfileDetail.class);
	...
	private void serverResponse(String serviceCode, String statusCode, String respCode, String message, JSONObject dataJson)
	{
		...
		try {
			...
			log.logMessage("GetProfileDetail Response :" + as, "info", GetProfileDetail.class);
