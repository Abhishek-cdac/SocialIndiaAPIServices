...
public class FacilityMasterList extends ActionSupport {
	Log log=new Log();	
	private String ivrparams;
	otpDao otp=new otpDaoService();
	InfoLibraryDao infoLibrary=new InfoLibraryDaoServices();
	FacilityDao facilityhbm=new FacilityDaoServices();
	List<FacilityMstTblVO> facilityList=new ArrayList<FacilityMstTblVO>();
	...
	public String execute(){
		...
		/*	if(facilityId!=null && facilityId.length()>0){
				search+=" and facilityId='"+facilityId+"'";
			}*/
			
				int societyId=userMst.getSocietyId().getSocietyId();
				locVrSlQry="from FacilityMstTblVO where statusFlag=1 and entryDatetime<STR_TO_DATE('" + timestamp + "','%Y-%m-%d %H:%i:%S')  and societyKey='"+societykey+"' "+search;
				facilityList=facilityhbm.getFacilityMasterList(locVrSlQry,startlimit,getText("total.row"),timestamp);
				System.out.println("=========userSkillList======="+facilityList.size());
				JSONObject finalJsonarr=new JSONObject();
				locObjRspdataJson=new JSONObject();
				JSONArray jArray =new JSONArray();
				if( facilityList!=null && facilityList.size()>0){
					FacilityMstTblVO facilityObj;
					for(Iterator<FacilityMstTblVO> it=facilityList.iterator();it.hasNext();){
						facilityObj=it.next();
						finalJsonarr=new JSONObject();
						finalJsonarr.put("facility_id", facilityObj.getFacilityId());
						finalJsonarr.put("facility_name", facilityObj.getFacilityName());
						//finalJsonarr.put("description", facilityObj.getDescription());
						jArray.put(finalJsonarr);
					}
					
				locObjRspdataJson.put("facilities", jArray);
				//locObjRspdataJson.put("totalrecords",facilityList.size());
