...
public class signUpMobile extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		System.out.println("**********sign up mobile******************");
		...
		if(flg){
			...
			if(result==true){
				...
				if( userInfo!=null ){
					...
					/*boolean checkUserActive=signup.checkUserActSts(mobileno);
					if(checkUserActive==true){
						locObjRspdataJson=new JSONObject();
						serverResponse(servicecode,"01","R0032",getText("R0032"),locObjRspdataJson);
					}else{*/
					...
					if(accessResult==true){
						...
						System.out.println("============dfdsdsfds===userResult======="+userResult);
						if(userResult==true){
							System.out.println("==========w=ffffffffff=======dfdsdsfds==========");
							System.out.println("=============userResult=======dfdsdsfds=========="+accessResult);
							System.out.println("=============userResult=======dfdsdsfds======xzzzz====");
						...
						if(userOtpInfo!=null && userInfoList.size()==1 ){
							
							
							    if(userInfo.getParentId()!=null && userInfo.getParentId()>0){
							    	password="";
							    }else{
							    	 updateResult=signup.updateOtpUser(mobileno, "",password,otpcount);
						          ...
						          try {
						           ...
						           +" FROM UserMasterTblVo as cust where cust.userId='"  + userInfo.getUserId() + "'";
						            ...
						            sms.setSmsEntryDateTime(common
						                .getStrCurrentDateTime("yyyy-MM-dd HH:mm:ss"));
						            System.out.println("=f=gf=g==="+sms.getSmsEntryDateTime());
									...
									JSONObject finalJsonarr=new JSONObject();
									 JSONArray jArray =new JSONArray();
										...
										for (Iterator<UserMasterTblVo> it = userInfoList
											...
											finalJsonarr.put("societylogo", System.getenv("SOCIAL_INDIA_BASE_URL")  +"/templogo/society/mobile/"+userObj.getUserId()+"/"+userObj.getSocietyId().getLogoImage());//need to doo
												...
												finalJsonarr.put("societylogo", System.getenv("SOCIAL_INDIA_BASE_URL")  +"/templogo/township/mobile/"+userObj.getUserId()+"/"+userObj.getSocietyId().getTownshipId().getTownshiplogoname());//need to doo
											...
											if(userData.getImageNameMobile()!=null && !userData.getImageNameMobile().equalsIgnoreCase("")){
												finalJsonarr.put("ppr_profilepic_thumbnail", System.getenv("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/mobile/"+userData.getUserId()+"/"+userData.getImageNameMobile());
												finalJsonarr.put("ppr_profilepic", System.getenv("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/mobile/"+userData.getUserId()+"/"+userData.getImageNameMobile());
				...
				//	}
					...
					locObjRspdataJson=new JSONObject();
					//locObjRspdataJson.put("otpfor","3");
			...
			log.logMessage("Service code : ivrservicecode, Sorry, signUpMobile an unhandled error occurred", "info", mobRequestOtp.class);
			locObjRspdataJson=new JSONObject();
	...
	private void serverResponse(String serviceCode, String statusCode, String respCode, String message, JSONObject dataJson)
	{
		...
		try {
			...
			System.out.println("======as=final response=="+as);
