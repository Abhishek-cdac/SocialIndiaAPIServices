...
public class myWishlistAdd extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		try{
			...
			if (ivIsJson) {		
				...
				jsonarr =(JSONArray) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson,"category_ids");
			...
			if(result==true){
				...
				if(societyKeyCheck==true){
					for(int i=0;i<jsonarr.length();i++){
						...
						wishListMst=new MvpWishlistTbl();
						categoryId=null;
						categoryId=jsonarr.getString(i);
						System.out.println("==categoryId==="+categoryId);
				boolean checkWishlist=wishlist.checkWishExist(rid,categoryId);
				System.out.println("===checkWishlist==="+checkWishlist);
					if(checkWishlist==false){
				
				categoryMst.setiVOCATEGORY_ID(Integer.parseInt(categoryId));
				userInfoMst.setUserId(Integer.parseInt(rid));
				wishListMst.setiVOCATEGORY_ID(categoryMst);
				wishListMst.setUserId(userInfoMst);
				wishListMst.setActFlg(1);
				wishListMst.setEnrtyDatetime(date1);
				
				resultflg=wishlist.insertWishList(wishListMst);
					}
					}
				if(resultflg==true){
				...
				System.out.println("=====timestamptimestamptimestamp==="+timestamp);
				...
				for (Iterator<MvpWishlistTbl> it = wishList
					...
					if(wishObj.getiVOCATEGORY_ID()!=null && wishObj.getiVOCATEGORY_ID().getiVOCATEGORY_NAME()!=null){
					...
					if(wishObj.getiVOCATEGORY_ID()!=null && wishObj.getiVOCATEGORY_ID().getCategoryDescription()!=null){
						finalJsonarr.put("category_desc", wishObj.getiVOCATEGORY_ID().getCategoryDescription());
					}else{
						finalJsonarr.put("category_desc", "");
					}
					if(wishObj.getiVOCATEGORY_ID()!=null && wishObj.getiVOCATEGORY_ID().getCategoryImageName()!=null){
						finalJsonarr.put("category_img_url",  System.getenv("SOCIAL_INDIA_BASE_URL")  +"/templogo/category/mobile/"+wishObj.getiVOCATEGORY_ID().getCategoryImageName());
					}else{
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : "+servicecode+", Request values are not correct format", "info", myWishlistAdd.class);
			serverResponse(servicecode,"01","R0016",mobiCommon.getMsg("R0016"),locObjRspdataJson);
		}
				
		}catch(Exception ex){
			System.out.println("=======myWishlistAdd====Exception===="+ex);
			log.logMessage("Service code : ivrservicecode, Sorry, myWishlistAdd an unhandled error occurred", "info", myWishlistAdd.class);
			locObjRspdataJson=new JSONObject();
