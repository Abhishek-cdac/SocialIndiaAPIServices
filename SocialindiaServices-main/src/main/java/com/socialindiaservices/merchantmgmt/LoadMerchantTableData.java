...
public class LoadMerchantTableData  extends ActionSupport {
	  /**
	   *sdsd.
	   */
	  ...
	  public String execute() {
	    ...
	    try {
	    	  ...
	    	  if (ivIsJson) {
	    		...
	    		if(ivrservicecode!=null && !ivrservicecode.equalsIgnoreCase("null") && !ivrservicecode.equalsIgnoreCase("")){
	    			...
	    			if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
	    				AuditTrial.toWriteAudit(getText("MCH0002"), "MCH0002", ivrCurrntusrid);
	    				serverResponse("SI6418","0","E8006",getText("mrchnt.data.select.error"),locObjRspdataJson);
	    			}else{
	    				AuditTrial.toWriteAudit(getText("MCH0001"), "MCH0001", ivrCurrntusrid);
	    				serverResponse("SI6418","0","S8006",getText("mrchnt.data.select.success"),locObjRspdataJson);					
	    			}
	    		}else {
	    			locObjRspdataJson=new JSONObject();
	    	    	logWrite.logMessage("Service code : SI6418,"+getText("request.values.empty")+"", "info", LoadMerchantTableData.class);
	    			serverResponse("SI6418","1","ER0001",getText("request.values.empty"),locObjRspdataJson);
	    	    }			    	   
	    	  }else {
	          locObjRspdataJson=new JSONObject();
	          logWrite.logMessage("Service code : SI6418,"+getText("request.format.notmach")+"", "info", LoadMerchantTableData.class);
			  serverResponse("SI6418","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);  
	    	  }
	      }else {
	    	locObjRspdataJson=new JSONObject();
	    	logWrite.logMessage("Service code : SI6418,"+getText("request.values.empty")+"", "info", LoadMerchantTableData.class);
	      ...
	      locObjRspdataJson=new JSONObject();
	      logWrite.logMessage("Service code : SI6418, Sorry, an unhandled error occurred : "+expObj, "error", LoadMerchantTableData.class);
		  serverResponse("SI6418","2","ER0002",getText("catch.error"),locObjRspdataJson);
		} finally {
	  ...
	  private JSONObject toloadMerchant(JSONObject praDatajson, Session praSession){	
	    ...
	    try {
	    	...
	    	System.out.println("Step 1 : Select Merchant Data Load start.");
	    	logWrite.logMessage("Step 1 : Select Merchant Data Load start.", "info", LoadMerchantTableData.class);
...
*/			}
			...
			if (locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))) {
				lvrevntcountqry = "select count(mrchntId) from MerchantTblVO where mrchntActSts=1"+searchField;
			...
			lvSlqry="from MerchantTblVO where mrchntActSts=1 "+searchField+" "+locOrderby;
			...
			while ( lvrObjeventlstitr.hasNext() ) {
				...
				lvrInrJSONObj.put("entryDatetime", Commonutility.toCheckNullEmpty(common.dateToString(merchantTblObj.getModifyDatetime(), rb.getString("date.Time.format"))));
				}else{
					lvrInrJSONObj.put("entryDatetime", "");
				}
				if(merchantTblObj.getModifyDatetime()!=null){
					lvrInrJSONObj.put("modifyDatetime", Commonutility.toCheckNullEmpty(common.dateToString(merchantTblObj.getModifyDatetime(), rb.getString("date.Time.format"))));
				}else{
