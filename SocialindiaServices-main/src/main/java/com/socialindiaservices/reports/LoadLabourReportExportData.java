...
public class LoadLabourReportExportData   extends ActionSupport {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private ArrayList labourList = new ArrayList();
	...
	public String execute(){
		...
		try{
			Map sessionMap = ActionContext.getContext().getSession();
			 if (ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0) {
				 ...
				 if (ivIsJson) {
					...
					String authtable = "from LaborDetailsTblVO where ivrBnLBR_STS=1 "+manualsearch+" order by modifyDatetime desc";			
					labourList = (ArrayList) reportDao.selectstaffPdf(authtable);
			
					String lvrJrxml = ServletActionContext.getServletContext().getRealPath("/resources/jsp/PDFCreation/reports/labourreport/labourreportPDF.jrxml");
					String lvrJasper = ServletActionContext.getServletContext().getRealPath("/resources/jsp/PDFCreation/reports/labourreport/labourreportPDF.jasper");			
					...
					JRBeanCollectionDataSource dataSource = new JRBeanCollectionDataSource(labourList);
			...
			AuditTrial.toWriteAudit(getText("LBADPDF002"), "LBADPDF002", 2);			
			return "input";
		} finally {
			 locObjRecvJson = null;
			 ivrservicecode=null;
		...
		try {
			...
			if (ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0) {
				...
				if (ivIsJson) {
					...
					String authtable = "from LaborDetailsTblVO where ivrBnLBR_STS=1 "+manualsearch+" order by modifyDatetime desc";			
					labourList= (ArrayList) reportDao.selectstaffPdf(authtable);		 			
					String lvrJrxml = ServletActionContext.getServletContext().getRealPath("/resources/jsp/PDFCreation/reports/labourreport/labourreportXLS.jrxml");
					String lvrJasper = ServletActionContext.getServletContext().getRealPath("/resources/jsp/PDFCreation/reports/labourreport/labourreportXLS.jasper");			
			...
			AuditTrial.toWriteAudit(getText("LBADXL002"), "LBADXL002", 2);			
			return "input";
		} finally {
			 locObjRecvJson = null;
			 ivrservicecode = null;
