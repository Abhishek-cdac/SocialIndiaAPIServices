...
public class LoadResidentReportTableData extends ActionSupport {
	  /**
	   *sdsd.
	   */
	  ...
	  public String execute() {
	    ...
	    try {
	    	  ...
	    	  if (ivIsJson) {
	    		...
	    		if(ivrservicecode!=null && !ivrservicecode.equalsIgnoreCase("null") && !ivrservicecode.equalsIgnoreCase("")){
	    			...
	    			if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
	    				AuditTrial.toWriteAudit(getText("RPT0008"), "RPT0008", ivrCurrntusrid);
	    				serverResponse("SI6429","0","E8006",getText("resident.rpt.view.error"),locObjRspdataJson);
	    			}else{
	    				//AuditTrial.toWriteAudit(getText("RPT0007"), "RPT0007", ivrCurrntusrid);
	    				serverResponse("SI6429","0","S8006",getText("resident.rpt.view.success"),locObjRspdataJson);					
	    			}
	    		}else {
	    			locObjRspdataJson=new JSONObject();
	    	    	logWrite.logMessage("Service code : SI6429,"+getText("request.values.empty")+"", "info", LoadResidentReportTableData.class);
	    			serverResponse("SI6429","1","ER0001",getText("request.values.empty"),locObjRspdataJson);
	    	    }			    	   
	    	  }else {
	          locObjRspdataJson=new JSONObject();
	          logWrite.logMessage("Service code : SI6429,"+getText("request.format.notmach")+"", "info", LoadResidentReportTableData.class);
			  serverResponse("SI6429","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);  
	    	  }
	      }else {
	    	locObjRspdataJson=new JSONObject();
	    	logWrite.logMessage("Service code : SI6429,"+getText("request.values.empty")+"", "info", LoadResidentReportTableData.class);
	      ...
	      locObjRspdataJson=new JSONObject();
	      AuditTrial.toWriteAudit(getText("RPT0008"), "RPT0008", ivrCurrntusrid);
	      logWrite.logMessage("Service code : SI6429, Sorry, an unhandled error occurred : "+expObj, "error", LoadResidentReportTableData.class);
		  serverResponse("SI6429","2","ER0002",getText("catch.error"),locObjRspdataJson);
		} finally {
	  ...
	  private JSONObject toloadMerchant(JSONObject praDatajson, Session praSession){	
	    ...
	    try {
	    	...
	    	logWrite.logMessage("Step 1 : Select Event process start.", "info", LoadResidentReportTableData.class);
			...
			System.out.println("sTowshipId-------------"+sTowshipId);
			if(sSoctyId>0){
			...
			if (locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))) {
				...
				logWrite.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+lvrevntcountqry, "info", LoadResidentReportTableData.class);
				...
				logWrite.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", LoadResidentReportTableData.class);
			}
			
			
			System.out.println("Step 3 : Resident Details Query : "+lvSlqry);
			logWrite.logMessage("Step 3 : Resident Details Query : "+lvSlqry, "info", LoadResidentReportTableData.class);
			...
			logWrite.logMessage("Step 4 : Select Resident process End", "info", LoadResidentReportTableData.class);
	    return lvrRtnjsonobj;
	    }catch(Exception expObj) {
	    	try {
				System.out.println("Exception in toResidentselect() : "+expObj);
				logWrite.logMessage("Step -1 : Resident select all block Exception : "+expObj, "error", LoadResidentReportTableData.class);	
				lvrRtnjsonobj=new JSONObject();
				lvrRtnjsonobj.put("InitCount", count);
				lvrRtnjsonobj.put("countAfterFilter", countFilter);
