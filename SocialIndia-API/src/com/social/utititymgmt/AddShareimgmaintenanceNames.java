...
public class AddShareimgmaintenanceNames extends ActionSupport{
		...
		try {
			...
			if (ivrparams != null && !ivrparams.equalsIgnoreCase("null")
				...
				if (ivIsJson) {
					...
					if(pastDocStareData!=null && pastDocStareData.size()>0){
						...
						for(Iterator<DocumentShareTblVO> it=pastDocStareData.iterator();it.hasNext();){
							...
							if(j==0){
								...
								oldmobPath=rb.getString("external.documnet.fldr")+rb.getString("external.documnet.private.fldr")+rb.getString("external.documnet.maintenancedoc.fldr")+rb.getString("external.inner.mobilepath")+pastoneuserId+"/"+pastDacmngData.getDocDateFolderName();
								oldfile=pastDacmngData.getDocFileName();
								doctempPathweb=rb.getString("external.documnet.fldr")+"temp/web";
								doctempPathmobile=rb.getString("external.documnet.fldr")+"temp/mobile";
								common.createDirIfNotExist(doctempPathweb);
								common.createDirIfNotExist(doctempPathmobile);
								Commonutility.toFileMoving(oldpath, doctempPathweb, oldfile, "NO");
								Commonutility.toFileMoving(oldmobPath, doctempPathmobile, oldfile, "NO");
							}
							...
							newmobPath=rb.getString("external.documnet.fldr")+rb.getString("external.documnet.private.fldr")+rb.getString("external.documnet.maintenancedoc.fldr")+rb.getString("external.inner.mobilepath")+pastoneuserId+"/"+docFolderDate;
							newfile=pastDacmngData.getDocFileName();
							common.deleteIfFileExist(oldpath, oldfile);
							common.deleteIfFileExist(oldmobPath, oldfile);
							Commonutility.toFileMoving(doctempPathweb, newpath, newfile, "NO");
							Commonutility.toFileMoving(doctempPathmobile, newmobPath, newfile, "NO");
							j++;
						}
					}
					...
					if (docsharsiz > 0) {
						for (int dk = 0; dk < docsharsiz; dk++) {
							...
							common.createDirIfNotExist(maintfolder+"/mobile");
							common.createDirIfNotExist(maintfolder+"/web"+userpath);
							common.createDirIfNotExist(maintfolder+"/mobile"+userpath);
							...
							String tocremobilefolder=rb.getString("external.documnet.fldr")+rb.getString("external.documnet.private.fldr")+rb.getString("external.documnet.maintenancedoc.fldr")+rb.getString("external.inner.mobilepath")+userd+"/"+docFolderDate;
							...
							if(isShared){
								...
								Commonutility.toFileMoving(doctempPathmobile, tocremobilefolder, newfile, "No");
					...
					common.deleteIfFileExist(doctempPathmobile, newfile);
					System.out.println("ivrservicecode---------------"
							+ ivrservicecode);
					...
					locObjRspdataJson = new JSONObject();
					log.logMessage("Service code : SI6413,"
							+ getText("request.format.notmach") + "", "info",
							NewGeneralDocument.class);
					serverResponse("SI6413", "1", "EF0001",
				...
				log.logMessage("Service code : SI6413,"
						...
						NewGeneralDocument.class);
				serverResponse("SI6413", "1", "ER0001",
			...
			locObjRspdataJson = new JSONObject();
			log.logMessage(
					"Service code : SI6413, Sorry, an unhandled error occurred",
					"error", NewGeneralDocument.class);
			serverResponse("SI6413", "2", "ER0002", getText("catch.error"),
					locObjRspdataJson);
		} finally {
