...
public class BillMaintenanceDocumentUploadEngine extends Thread{
	...
	BillMaintenanceDocumentUploadEngine(String checksum,int userId,int societyCode,int pCurntGrpCode,String file_name,String lvrJrxml){
		logWrite = new Log();
	...
	public void run() {
		...
		Commonutility.toWriteConsole("Step 1 : [BillMaintenanceDocumentUploadEngine] Thread Started.");
		logWrite.logMessage("Step 1 : [BillMaintenanceDocumentUploadEngine] Thread Started.", "info", BillMaintenanceDocumentUploadEngine.class);
		String rsss = togetMaintenance();
		if (rsss !=null && rsss.equalsIgnoreCase("done") && this.isAlive()){
			Commonutility.toWriteConsole("Step 3 : [BillMaintenanceDocumentUploadEngine] Thread Stoped.");
			this.stop();
		} else {
			this.stop();
		}
...
//							totalAmount+=totbillamt;
			...
			Commonutility.toWriteConsole("BillMaintenanceDocumentUploadEngine ----- Exception -0----"+ ex);
		} finally {
