			...
			String qry="";
				...
				if(networkListObj!=null && networkListObj.size()>0){
					...
					for(Iterator<Object[]> it=networkListObj.iterator();it.hasNext();) {
						...
						/*if((String)objList[3]!=null){
							finalJsonarr.put("from_nwuser_img_url", getText("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/mobile/"+userid+"/"+(String)objList[3]);
						}
						...
						if((Integer)objList[8]!=null){
							...
							if(userInfo.getImageNameMobile()!=null){
								finalJsonarr.put("from_nwuser_img_url", getText("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/mobile/"+objList[8]+"/"+userInfo.getImageNameMobile());
						...
						if(userInfo.getSocietyId()!=null && userInfo.getSocietyId().getSocietyName()!=null && userInfo.getSocietyId().getSocietyName().length()>0){
							...
							if(userInfo.getImageNameMobile()!=null){
								finalJsonarr.put("to_nwuser_img_url", getText("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/mobile/"+userid+"/"+userInfo.getImageNameMobile());
								}else if(userInfo.getSocialProfileUrl()!=null){
						...
						if((Integer)objList[9]!=null){
							...
							finalJsonarr.put("to_nwuser_name",Commonutility.toCheckNullEmpty(tousername));
							if(userInfo.getImageNameMobile()!=null){
								finalJsonarr.put("to_nwuser_img_url", getText("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/mobile/"+objList[9]+"/"+userInfo.getImageNameMobile());
								}else if(userInfo.getSocialProfileUrl()!=null){
									finalJsonarr.put("to_nwuser_img_url", userInfo.getSocialProfileUrl());	
								}else{
									finalJsonarr.put("to_nwuser_img_url", "");	
				...
				for (Iterator<MvpNetworkTbl> it = networkMstList
					...
					if(networkObj.getUsrConnectedUsrId().getSocietyId().getLogoImage()!=null){
						finalJsonarr.put("nwuser_img_url", getText("SOCIAL_INDIA_BASE_URL")  +"/templogo/profile/web/"+networkObj.getUsrConnectedUsrId().getUserId()+"/"+networkObj.getUsrConnectedUsrId().getSocietyId().getLogoImage());
						}else{
