...
public class PrivacyUpdate extends ActionSupport{
	...
	public String execute() {
		...
		try {
			...
			String users = null;
			log.logMessage("Enter into PrivacyUpdate ", "info", PrivacyUpdate.class);
			if (Commonutility.checkempty(ivrparams)) {
				ivrparams = EncDecrypt.decrypt(ivrparams);
				log.logMessage("PrivacyUpdate ivrparams :" + ivrparams, "info", PrivacyUpdate.class);
...
//							}
			...
			log.logMessage("flg :" +flg, "info", PrivacyUpdate.class);
			if (flg) {
				...
				if(flg){
					...
					if (flg) {
						...
						if (privacy != 0) {
							if (privacy == 4) {
								privacyObj.setUsrIds(users);
							}
							UserMasterTblVo userMsterObj = new UserMasterTblVo();
							userMsterObj.setUserId(rid);
							privacyObj.setUsrId(userMsterObj);
							privacyObj.setPrivacyFlg(privacy);
							privacyObj.setStatus(1);
							flg = commonServ.updatePrivacyFlg(privacyObj);
						} else {
							flg = false;
						}
						if (flg) {
							serverResponse(ivrservicecode,getText("status.success"),"R0207",mobiCommon.getMsg("R0207"),locObjRspdataJson);
						} else {
							serverResponse(ivrservicecode,getText("status.warning"),"R0208",mobiCommon.getMsg("R0208"),locObjRspdataJson);
						}
					} else {
						locObjRspdataJson=new JSONObject();
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage(getText("Eex") + ex, "error", PrivacyUpdate.class);
