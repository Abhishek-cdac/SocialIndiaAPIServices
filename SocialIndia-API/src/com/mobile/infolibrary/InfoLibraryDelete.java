...
import com.mobile.familymember.familyMemberDelete;
...
public class InfoLibraryDelete extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		String doc_id = (String) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson, "doc_id");
		...
		if (locObjRecvdataJson !=null) {
			if (Commonutility.checkempty(doc_id)) {
			} else {
				flg = false;
				locErrorvalStrBuil.append(Commonutility.validateErrmsgForm(getText("docId.error")));
			}
		}
		...
		if(flg){
			...
			if(result==true){
				...
				if(societyKeyCheck==true){
					System.out.println("rid===="+rid+"----docid----"+doc_id);
				boolean delResult=infoLibrary.deleteInfoDocument(rid,doc_id);
