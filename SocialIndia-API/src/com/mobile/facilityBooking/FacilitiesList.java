...
public class FacilitiesList extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		try{
			...
			int count=0;String locVrSlQry="";
				...
				locVrSlQry="from FacilityBookingTblVO where bookingStatus=1 and bookedBy="+rid;
				facilityList=facilityhbm.getBookedFacilityList(locVrSlQry);
				...
				if( facilityList!=null && facilityList.size()>0){
					...
					for(Iterator<FacilityBookingTblVO> it=facilityList.iterator();it.hasNext();){
						...
						facilityObj=it.next();
						finalJsonarr.put("facility_booking_id", ""+facilityObj.getBookingId());
						finalJsonarr.put("facility_booking_title", facilityObj.getTitle());
