...
public class updateFamilyRelationship extends ActionSupport {
	Log log=new Log();	
	...
	public String execute(){
		...
		System.out.println("**********update FamilyRelationship services******************");
		...
		try{
			...
			System.out.println("====societykey==="+societykey);
			System.out.println("====rid==="+rid);
			boolean societyKeyCheck=otp.checkSocietyKey(societykey,rid);
			if(societyKeyCheck==true){
			
				
				boolean upResult=family.updateFamilyRelationInfo(rid,fm_uid,relationship_id);
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : "+servicecode+" Request values are not correct format", "info", updateFamilyRelationship.class);
			serverResponse(servicecode,"01","R0016",mobiCommon.getMsg("R0016"),locObjRspdataJson);
		}
		}catch(Exception ex){
			System.out.println("=======updateFamilyRelationship====Exception===="+ex);
			log.logMessage("Service code : ivrservicecode, Sorry, updateFamilyRelationship an unhandled error occurred", "info", updateFamilyRelationship.class);
			locObjRspdataJson=new JSONObject();
