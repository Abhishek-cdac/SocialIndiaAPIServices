...
public class LoadCommitteeReportTableData extends ActionSupport {
	  /**
	   *sdsd.
	   */
	  ...
	  public String execute() {
	    ...
	    try {
	      ...
	      if (ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0) {
	    	  ...
	    	  if (ivIsJson) {
	    		...
	    		if(ivrservicecode!=null && !ivrservicecode.equalsIgnoreCase("null") && !ivrservicecode.equalsIgnoreCase("")){
	    			...
	    			if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
	    				AuditTrial.toWriteAudit(getText("RPT0004"), "RPT0004", ivrCurrntusrid);
	    				serverResponse("SI6431","0","E8006",getText("committee.rpt.view.error"),locObjRspdataJson);
	    			}else{
	    				//AuditTrial.toWriteAudit(getText("RPT0003"), "RPT0003", ivrCurrntusrid);
	    				serverResponse("SI6431","0","S8006",getText("committee.rpt.view.success"),locObjRspdataJson);					
	    			}
	    		}else {
	    			locObjRspdataJson=new JSONObject();
	    	    	logWrite.logMessage("Service code : SI6431,"+getText("request.values.empty")+"", "info", LoadCommitteeReportTableData.class);
	    			serverResponse("SI6431","1","ER0001",getText("request.values.empty"),locObjRspdataJson);
	    	    }			    	   
	    	  }else {
	          locObjRspdataJson=new JSONObject();
	          logWrite.logMessage("Service code : SI6431,"+getText("request.format.notmach")+"", "info", LoadCommitteeReportTableData.class);
			  serverResponse("SI6431","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);  
	    	  }
	      }else {
	    	locObjRspdataJson=new JSONObject();
	    	logWrite.logMessage("Service code : SI6431,"+getText("request.values.empty")+"", "info", LoadCommitteeReportTableData.class);
	    	...
	    	AuditTrial.toWriteAudit(getText("RPT0004"), "RPT0004", ivrCurrntusrid);
	      locObjRspdataJson=new JSONObject();
	      logWrite.logMessage("Service code : SI6431, Sorry, an unhandled error occurred : "+expObj, "error", LoadCommitteeReportTableData.class);
		  serverResponse("SI6431","2","ER0002",getText("catch.error"),locObjRspdataJson);
		} finally {
	  ...
	  private JSONObject toloadMerchant(JSONObject praDatajson, Session praSession){	
	    ...
	    try {
	    	...
	    	logWrite.logMessage("Step 1 : Select Event process start.", "info", LoadCommitteeReportTableData.class);
...
*/			}
			...
			if (locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))) {
				lvrevntcountqry = "select count(userId) from UserMasterTblVo where statusFlag=1 and groupCode="+locGrpcode+" "+searchField+manualsearch;
				System.out.println("Step 2 : Count / Filter Count block enter SlQry : "+lvrevntcountqry);
				logWrite.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+lvrevntcountqry, "info", LoadCommitteeReportTableData.class);
				...
				logWrite.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", LoadCommitteeReportTableData.class);
			}
			...
			String locOrderby =" order by userId desc";						
			lvSlqry="from UserMasterTblVo where statusFlag=1 and groupCode="+locGrpcode+" "+searchField+manualsearch+" "+locOrderby;
			System.out.println("Step 3 : Resident Details Query : "+lvSlqry);
			logWrite.logMessage("Step 3 : Resident Details Query : "+lvSlqry, "info", LoadCommitteeReportTableData.class);
			...
			while ( lvrObjeventlstitr.hasNext() ) {
				...
				//
				int accessChnl = residentTblObj.getAccessChannel();
				if ( accessChnl == 1){
					lvrInrJSONObj.put("accessChannel", "Web");
				}
				else if ( accessChnl == 2){
					lvrInrJSONObj.put("accessChannel", "Mobile");
				}
				else{
					lvrInrJSONObj.put("accessChannel", "Both");
				}
				
				String comtRole = "";
				if (residentTblObj.getRoleId() != null && residentTblObj.getRoleId().getRoleName() != null){
					comtRole = residentTblObj.getRoleId().getRoleName();
				}
				lvrInrJSONObj.put("committeeRole", comtRole);
			...
			System.out.println("Step 4 : Select Resident process End");
			logWrite.logMessage("Step 4 : Select Resident process End", "info", LoadCommitteeReportTableData.class);
	    return lvrRtnjsonobj;
	    }catch(Exception expObj) {
	    	try {
	    		String error = "";
	    		StackTraceElement arr[] = expObj.getStackTrace();
	    		for (int i = 0; i < arr.length; i++) {
					error = error + arr[i];
				}
				System.out.println("Exception in toResidentselect() : "+error);
				logWrite.logMessage("Step -1 : Resident select all block Exception : "+expObj, "error", LoadCommitteeReportTableData.class);	
				lvrRtnjsonobj=new JSONObject();
				lvrRtnjsonobj.put("InitCount", count);
				lvrRtnjsonobj.put("countAfterFilter", countFilter);
