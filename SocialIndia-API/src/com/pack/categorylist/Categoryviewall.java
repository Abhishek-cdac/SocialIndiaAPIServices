...
public class Categoryviewall extends ActionSupport {
	...
	public String execute(){
		...
		try{
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				locObjRspdataJson=toCategorySelectAll(locObjRecvdataJson,locObjsession);	
				String errocheck=(String) Commonutility.toHasChkJsonRtnValObj(locObjRspdataJson, "CatchBlock");// if Catch block found error occurred in select
					if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
						...
						serverResponse("SI15000","0","E15000",getText("Category.selectall.error"),locObjRspdataJson);
					}else{
						AuditTrial.toWriteAudit(getText("IDCARDAD013"), "IDCARDAD013", ivrCurrntusrid);
						serverResponse("SI15000","0","S15000",getText("Category.selectall.success"),locObjRspdataJson);					
					}
				
				}else{
					locObjRspdataJson=new JSONObject();
					log.logMessage("Service code : SI15000, "+getText("request.format.notmach"), "info", Categoryviewall.class);
					serverResponse("SI15000","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);

				}	
			}else{
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI15000, "+getText("request.values.empty"), "info", Categoryviewall.class);
				serverResponse("SI15000","1","ER0001",getText("request.values.empty"),locObjRspdataJson);

			}	
		}catch(Exception e){
			System.out.println("Exception found .class execute() Method : "+e);
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : SI15000, "+getText("catch.error"), "error", Categoryviewall.class);
			serverResponse("SI15000","2","ER0002",getText("catch.error"),locObjRspdataJson);
		}finally{
	...
	public static JSONObject toCategorySelectAll(JSONObject pDataJson, Session pSession) {
		...
		CategoryMasterTblVO lbrDtlVoObj=null;
		int count=0,countFilter=0, startrowfrom=1, totalNorow=10;		
		try {
			log=new Log();	lbrDtlVoObj=new CategoryMasterTblVO();			
			System.out.println("Step 1 : category select all block enter");
			log.logMessage("Step 1 : category select all block enter", "info", Categoryviewall.class);			
			...
			if(locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))){// for web 
				...
				{
					locvrIDCard_cntQry="select count(iVOCATEGORY_ID) from CategoryMasterTblVO where iVOACT_STAT in(0,1)";
				}
				else
				{
					locvrIDCard_cntQry="select count(iVOCATEGORY_ID) from CategoryMasterTblVO where iVOACT_STAT in(0,1) AND (" + "iVOCATEGORY_NAME like ('%" + loctocheNull+ "%')  " 
			                                + ")";
				}
				System.out.println("Step 2 : Count / Filter Count block enter SlQry : "+locvrIDCard_cntQry);
				log.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+locvrIDCard_cntQry, "info", Categoryviewall.class);
				...
				countFilter=IdcardDaoObj.getTotalFilter(locvrIDCard_cntQry);
				 ...
				 log.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", Categoryviewall.class);
			}
				...
				String lvrOrdeby =" order by iVOCATEGORY_ID desc";
			if(loctocheNull!=null && !loctocheNull.equalsIgnoreCase("null") && !loctocheNull.equalsIgnoreCase("")){
				  globalsearch = " AND (" + "iVOACT_STAT like ('%" + loctocheNull+ "%') or " 
			                                + "iVOCATEGORY_NAME like ('%" + loctocheNull + "%'))";
				  loc_slQry="from CategoryMasterTblVO  where iVOACT_STAT in(0,1) " +globalsearch +" "+lvrOrdeby;
			}else{
				loc_slQry="from CategoryMasterTblVO  where iVOACT_STAT in(0,1) " + lvrOrdeby;	
			}				
			//loc_slQry="from LaborDetailsTblVO  where ivrBnLBR_STS="+locvrLBR_STS+"";	
			System.out.println("Step 3 : category Details Query : "+loc_slQry);
			log.logMessage("Step 3 : category Details Query : "+loc_slQry, "info", Categoryviewall.class);
			...
			while ( locObjLbrlst_itr.hasNext() ) {
				...
				lbrDtlVoObj = (CategoryMasterTblVO) locObjLbrlst_itr.next();								
				locInrJSONObj.put("category_id",lbrDtlVoObj.getiVOCATEGORY_ID());	
				locInrJSONObj.put("category_name",lbrDtlVoObj.getiVOCATEGORY_NAME());
				locInrJSONObj.put("category_status",lbrDtlVoObj.getiVOACT_STAT());									
			...
			locFinalRTNObj.put("categorydetails", locLBRJSONAryobj);			
			System.out.println("locFinalRTNObj : "+locFinalRTNObj);
			System.out.println("Step 6 : Select category detail block end.");
			log.logMessage("Step 4: Select category detail block end.", "info", Categoryviewall.class);
			return locFinalRTNObj;
		} catch (Exception e) {
			try{
			System.out.println("Exception in toCMPYSelectAll() : "+e);
			log.logMessage("Step -1 : category select all block Exception : "+e, "error", Categoryviewall.class);	
			locFinalRTNObj=new JSONObject();
			locFinalRTNObj.put("InitCount", count);
			locFinalRTNObj.put("countAfterFilter", countFilter);
			locFinalRTNObj.put("categorydetails", "");
