...
public class CommitteeroleDaoservice implements CommitteeroleDao{
	...
	public boolean saveLaborInfo(LaborProfileTblVO pIntLabrobj) {
		...
		try {
			...
			log.logMessage("Exception : "+e, "error", CommitteeroleDaoservice.class);
			return false;
		} finally {
	...
	public int toInsertcmtroletype(CommittteeRoleMstTbl cmtroletypeTblObj) {
		...
		try {
			session = HibernateUtil.getSessionFactory().openSession();
			tx = session.beginTransaction();			
			session.save(cmtroletypeTblObj);
			locidcardid=cmtroletypeTblObj.getRoleId();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
				tx.rollback();
			...
			log=new Log();
			System.out.println("Exception found in CommitteeroleDaoservice.class : "+e);
			log.logMessage("Exception : "+e, "error", CommitteeroleDaoservice.class);
			locidcardid=-1;
			return locidcardid;
		} finally {
	...
	public String toExistcommitteelistname(String lvrcategorytypetitle) {
		// TODO Auto-generated method stub
		  String query = "SELECT CASE WHEN COUNT(*)=0 THEN 'NEW' ELSE "
		          + "'ALREADY EXISTS' END FROM CommittteeRoleMstTbl where roleName='"
