...
public class merchantCategoryDaoservice implements merchantCategoryDao{
	...
	public boolean saveLaborInfo(LaborProfileTblVO pIntLabrobj) {
		...
		try {
			...
			System.out.println("Exception found in merchantCategoryDaoservice : "+e);
			log.logMessage("Exception : "+e, "error", merchantCategoryDaoservice.class);
			return false;
		} finally {
	...
	public int toInsertmerchantCategory(MerchantCategoryTblVO merchantCategoryTblObj) {
		...
		try {
			session = HibernateUtil.getSessionFactory().openSession();
			tx = session.beginTransaction();			
			session.save(merchantCategoryTblObj);
			locidcardid=merchantCategoryTblObj.getMrchCategoryId();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
				tx.rollback();
	...
	public String toExistmerchantCategorylistname(String lvrmerchantCategorytitle) {
		// TODO Auto-generated method stub
		  String query = "SELECT CASE WHEN COUNT(*)=0 THEN 'NEW' ELSE "
		          + "'ALREADY EXISTS' END FROM MerchantCategoryTblVO where mrchCategoryName='"
		          + lvrmerchantCategorytitle + "'";
		  Log log=null;
			Session session = null;
			Transaction tx = null;
			Query qy = null;
			...
			try {
				...
				log=new Log();
				System.out.println("Exception found in merchantCategoryDaoservice.class : "+e);
				log.logMessage("Exception : "+e, "error", merchantCategoryDaoservice.class);
