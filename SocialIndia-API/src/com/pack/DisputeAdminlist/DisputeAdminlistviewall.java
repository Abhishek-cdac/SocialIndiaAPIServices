...
public class DisputeAdminlistviewall extends ActionSupport {
	...
	public String execute(){
		...
		try{
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				locObjRspdataJson=toDisputeAdminlistSelectAll(locObjRecvdataJson,locObjsession,getText("Grp.social.admin"));	
				String errocheck=(String) Commonutility.toHasChkJsonRtnValObj(locObjRspdataJson, "CatchBlock");// if Catch block found error occurred in select
					if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
						AuditTrial.toWriteAudit(getText("DISPADMAD014"), "DISPADMAD014", ivrCurrntusrid);
						serverResponse("SI32000","0","E32000",getText("disputeadmin.selectall.error"),locObjRspdataJson);
					}else{
						AuditTrial.toWriteAudit(getText("DISPADMAD013"), "DISPADMAD013", ivrCurrntusrid);
						serverResponse("SI32000","0","S32000",getText("disputeadmin.selectall.success"),locObjRspdataJson);					
					}
				
				}else{
					locObjRspdataJson=new JSONObject();
					log.logMessage("Service code : SI32000, "+getText("request.format.notmach"), "info", DisputeAdminlistviewall.class);
					serverResponse("SI32000","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);

				}	
			}else{
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI32000, "+getText("request.values.empty"), "info", DisputeAdminlistviewall.class);
				serverResponse("SI32000","1","ER0001",getText("request.values.empty"),locObjRspdataJson);

			}	
		}catch(Exception e){
			System.out.println("Exception found .class execute() Method : "+e);
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : SI32000, "+getText("catch.error"), "error", DisputeAdminlistviewall.class);
			serverResponse("SI32000","2","ER0002",getText("catch.error"),locObjRspdataJson);
		}finally{
	...
	public static JSONObject toDisputeAdminlistSelectAll(JSONObject pDataJson, Session pSession,String pGrpName) {
		...
		UserMasterTblVo locLbrMstrVOobj=null,locLbrMstrvo=null;
		Log log=null;
		...
		try {
			...
			System.out.println("Step 1 : Disputesocialadmin Type  select all block enter");
			log.logMessage("Step 1 : Disputesocialadmin Type  select all block enter", "info", DisputeAdminlistviewall.class);			
			...
			if(locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))){// for web 
				...
				log.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+locvrEdu_cntQry, "info", DisputeAdminlistviewall.class);
				...
				countFilter=IdcardDaoObj.getTotalFilter(locvrEdu_cntQry);
				 ...
				 log.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", DisputeAdminlistviewall.class);
			}
			...
			System.out.println("Step 3 : Disputesocialadmin Type  Details Query : "+loc_slQry);
			log.logMessage("Step 3 : Disputesocialadmin Type  Details Query : "+loc_slQry, "info", DisputeAdminlistviewall.class);
			...
			while ( locObjLbrlst_itr.hasNext() ) {
				...
				String merchrentid =Commonutility.toCheckNullEmpty(lbrDtlVoObj.getDisputeT0Id());
				 locLbrSLqry="from UserMasterTblVo where userId ="+merchrentid+"";			 
				 locLbrQryrst=pSession.createQuery(locLbrSLqry);			
				 locLbrMstrVOobj=(UserMasterTblVo) locLbrQryrst.uniqueResult();
				 if(locLbrMstrVOobj!=null){
					 locLbrMstrvo=new UserMasterTblVo();
					 dispute_tomerchname=locLbrMstrVOobj.getUserName();
					 locInrJSONObj.put("disp_tosocialadminname",Commonutility.toCheckNullEmpty(dispute_tomerchname));					 
				 }
			...
			locFinalRTNObj.put("disputeadmindetails", locLBRJSONAryobj);			
			System.out.println("locFinalRTNObj : "+locFinalRTNObj);
			System.out.println("Step 6 : Select category detail block end.");
			log.logMessage("Step 4: Select category detail block end.", "info", DisputeAdminlistviewall.class);
			return locFinalRTNObj;
		} catch (Exception e) {
			try{
			System.out.println("Exception in toCMPYSelectAll() : "+e);
			log.logMessage("Step -1 : category select all block Exception : "+e, "error", DisputeAdminlistviewall.class);	
			locFinalRTNObj=new JSONObject();
			locFinalRTNObj.put("InitCount", count);
			locFinalRTNObj.put("countAfterFilter", countFilter);
			locFinalRTNObj.put("disputeadmindetails", "");
