...
public class SitoMerchantSendEmail extends ActionSupport {
	/**
	 * 
	 */
	...
	public String execute() throws Exception {
		...
		try {
			...
			if (ivIsJson) {
				...
				if(merchantData!=null && usrtblvo!=null){
					...
					//Send Email
						...
						if (merchantData.getMrchntEmail()!=null){
							...
							try {
								String emqry = "FROM EmailTemplateTblVo where tempName ='Share Complaint ToMerchant' and status ='1'";
								...
								String merchrentid = Commonutility.toCheckNullEmpty(merchantData.getMrchntId());
								String mrchname = Commonutility.toCheckNullEmpty(merchantData.getMrchntName());
								if (!Commonutility.checkempty(mrchname)) {
									mrchname = merchantData.getShopName();
								}
								//String lvrcmpldate = Commonutility.changedateformat(lvrMrchissu.getEntryDatetime(), "dd-MM-yyyy HH:mm:ss");
								...
								if (lvrMrchissu.getIssueTypes()!=null) {
									...
									for (int i = 0; i < splitval.length; i++) {
										 String grpcodeappend =" and ivrGrpcodeobj = 8 ";
										 String locMrchSLqry1="from MerchantIssueTblVO where issueId ="+splitval[i]+" and mcrctLaborId ="+merchrentid+" "+grpcodeappend+"";
										 Query locMrchQryrst=locObjsession.createQuery(locMrchSLqry1);
										 MerchantIssueTblVO  locMrchIssVOobj= (MerchantIssueTblVO) locMrchQryrst.uniqueResult();
								...
								emailMessage =  emailMessage.replace("[MERCHANTNAME]", mrchname);
			           ...
			           // String qry = qrySplit[0] + " FROM MerchantIssuePostingTblVO as mrchpost where mrchpost.issueId="+Disputeid+"";
			          ...
			          //  emailMessage =  emailMessage.replace("[COMMENTS]",commentval);
			            ...
			            emailfun.test2(merchantData.getMrchntEmail(), emailTemplate.getSubject(), emailMessage);
			            ...
			            log.logMessage("Exception in siSocietySendEmail admin flow emailFlow----> " + ex, "error",SitoMerchantSendEmail.class);
