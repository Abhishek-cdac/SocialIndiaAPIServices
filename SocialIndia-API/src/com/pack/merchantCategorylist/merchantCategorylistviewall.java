...
public class merchantCategorylistviewall extends ActionSupport {
	...
	public String execute(){
		...
		try{
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				locObjRspdataJson=toWorktypelistSelectAll(locObjRecvdataJson,locObjsession);	
				String errocheck=(String) Commonutility.toHasChkJsonRtnValObj(locObjRspdataJson, "CatchBlock");// if Catch block found error occurred in select
					if(errocheck!=null && errocheck.equalsIgnoreCase("Error")){//catch found
						AuditTrial.toWriteAudit(getText("MERCCATAD014"), "MERCCATAD014", ivrCurrntusrid);
						serverResponse("SI21000","0","E21000",getText("merchantCategory.selectall.error"),locObjRspdataJson);
					}else{
						AuditTrial.toWriteAudit(getText("MERCCATAD013"), "MERCCATAD013", ivrCurrntusrid);
						serverResponse("SI21000","0","S21000",getText("merchantCategory.selectall.success"),locObjRspdataJson);					
					}
				
				}else{
					locObjRspdataJson=new JSONObject();
					log.logMessage("Service code : SI21000, "+getText("request.format.notmach"), "info", merchantCategorylistviewall.class);
					serverResponse("SI21000","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);

				}	
			}else{
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI21000, "+getText("request.values.empty"), "info", merchantCategorylistviewall.class);
				serverResponse("SI21000","1","ER0001",getText("request.values.empty"),locObjRspdataJson);

			}	
		}catch(Exception e){
			System.out.println("Exception found .class execute() Method : "+e);
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : SI21000, "+getText("catch.error"), "error", merchantCategorylistviewall.class);
			serverResponse("SI21000","2","ER0002",getText("catch.error"),locObjRspdataJson);
		}finally{
	...
	public static JSONObject toWorktypelistSelectAll(JSONObject pDataJson, Session pSession) {
		JSONObject locFinalRTNObj=null;
		JSONObject locInrJSONObj=null;
		JSONArray locLBRJSONAryobj=null;
		
		String locvrLBR_STS=null,locvrCntflg=null,locvrFlterflg=null,locvrMCat_cntQry=null;
		String loc_slQry=null,locStrRow=null,locMaxrow=null, locsrchdtblsrch=null;	
		Log log=null;
		Iterator locObjLbrlst_itr=null;
		MerchantCategoryTblVO lbrDtlVoObj=null;
		int count=0,countFilter=0, startrowfrom=1, totalNorow=10;		
		try {
			log=new Log();	lbrDtlVoObj=new MerchantCategoryTblVO();			
			System.out.println("Step 1 : merchantCategory Type  select all block enter");
			log.logMessage("Step 1 : merchantCategory Type  select all block enter", "info", merchantCategorylistviewall.class);			
			...
			if(locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))){// for web 
				...
				{
					locvrMCat_cntQry="select count(mrchCategoryId) from MerchantCategoryTblVO where status in(0,1)";
				}
				else
				{
					locvrMCat_cntQry="select count(mrchCategoryId) from MerchantCategoryTblVO where status in(0,1) AND (" + "mrchCategoryName like ('%" + loctocheNull+ "%')  " 
			                                + ")";
				}
				System.out.println("Step 2 : Count / Filter Count block enter SlQry : "+locvrMCat_cntQry);
				log.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+locvrMCat_cntQry, "info", merchantCategorylistviewall.class);
				...
				countFilter=IdcardDaoObj.getTotalFilter(locvrMCat_cntQry);
				 ...
				 log.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", merchantCategorylistviewall.class);
			}
				...
				String globalsearch=null, lvrOrdby = "order by mrchCategoryDesc desc";
			if(loctocheNull!=null && !loctocheNull.equalsIgnoreCase("null") && !loctocheNull.equalsIgnoreCase("")){
				  globalsearch = " AND (" + "status like ('%" + loctocheNull+ "%') or " 
			                                + "mrchCategoryName like ('%" + loctocheNull + "%'))";
				  loc_slQry="from MerchantCategoryTblVO  where status in(0,1) " +globalsearch+" "+lvrOrdby;
			}else{
				loc_slQry="from MerchantCategoryTblVO  where status in(0,1) " +lvrOrdby;	
			}				
			//loc_slQry="from LaborDetailsTblVO  where ivrBnLBR_STS="+locvrLBR_STS+"";	
			System.out.println("Step 3 : merchantCategory Type  Details Query : "+loc_slQry);
			log.logMessage("Step 3 : merchantCategory Type  Details Query : "+loc_slQry, "info", merchantCategorylistviewall.class);
			locObjLbrlst_itr=pSession.createQuery(loc_slQry).setFirstResult(startrowfrom).setMaxResults(totalNorow).list().iterator();
			System.out.println("locObjLbrlst_itr=== : "+locObjLbrlst_itr);
					
			locLBRJSONAryobj=new JSONArray();
			while ( locObjLbrlst_itr.hasNext() ) {
				locInrJSONObj=new JSONObject();
				lbrDtlVoObj = (MerchantCategoryTblVO) locObjLbrlst_itr.next();								
				locInrJSONObj.put("merchantCategory_id",lbrDtlVoObj.getMrchCategoryId());	
				locInrJSONObj.put("merchantCategory_name",lbrDtlVoObj.getMrchCategoryName());
				locInrJSONObj.put("merchantCategory_status",lbrDtlVoObj.getStatus());									
			...
			locFinalRTNObj.put("merchantCategorydetails", locLBRJSONAryobj);			
			System.out.println("locFinalRTNObj : "+locFinalRTNObj);
			System.out.println("Step 6 : Select category detail block end.");
			log.logMessage("Step 4: Select category detail block end.", "info", merchantCategorylistviewall.class);
			return locFinalRTNObj;
		} catch (Exception e) {
			try{
			System.out.println("Exception in toCMPYSelectAll() : "+e);
			log.logMessage("Step -1 : category select all block Exception : "+e, "error", merchantCategorylistviewall.class);	
			locFinalRTNObj=new JSONObject();
			locFinalRTNObj.put("InitCount", count);
			locFinalRTNObj.put("countAfterFilter", countFilter);
			locFinalRTNObj.put("merchantCategorydetails", "");
