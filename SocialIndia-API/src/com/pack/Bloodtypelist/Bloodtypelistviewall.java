...
public class Bloodtypelistviewall extends ActionSupport {
	...
	public String execute(){
		...
		try{
			...
			if(ivrparams!=null && !ivrparams.equalsIgnoreCase("null") && ivrparams.length()>0){
				...
				locObjRspdataJson=toBloodtypelistSelectAll(locObjRecvdataJson,locObjsession);	
				String errocheck=(String) Commonutility.toHasChkJsonRtnValObj(locObjRspdataJson, "CatchBlock");// if Catch block found error occurred in select
					...
					//	AuditTrial.toWriteAudit(getText("WRKTYPAD014"), "WRKTYPAD014", ivrCurrntusrid);
						serverResponse("SI36000","0","E36000",getText("bloodtype.selectall.error"),locObjRspdataJson);
					}else{
						//AuditTrial.toWriteAudit(getText("WRKTYPAD013"), "WRKTYPAD013", ivrCurrntusrid);
						serverResponse("SI36000","0","S36000",getText("bloodtype.selectall.success"),locObjRspdataJson);					
					}
				
				}else{
					locObjRspdataJson=new JSONObject();
					log.logMessage("Service code : SI36000, "+getText("request.format.notmach"), "info", Bloodtypelistviewall.class);
					serverResponse("SI36000","1","EF0001",getText("request.format.notmach"),locObjRspdataJson);

				}	
			}else{
				locObjRspdataJson=new JSONObject();
				log.logMessage("Service code : SI36000, "+getText("request.values.empty"), "info", Bloodtypelistviewall.class);
				serverResponse("SI36000","1","ER0001",getText("request.values.empty"),locObjRspdataJson);

			}	
		}catch(Exception e){
			System.out.println("Exception found .class execute() Method : "+e);
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : SI36000, "+getText("catch.error"), "error", Bloodtypelistviewall.class);
			serverResponse("SI36000","2","ER0002",getText("catch.error"),locObjRspdataJson);
		}finally{
	...
	public static JSONObject toBloodtypelistSelectAll(JSONObject pDataJson, Session pSession) {
		...
		MvpBloodGroupTbl lbrDtlVoObj=null;
		int count=0,countFilter=0, startrowfrom=1, totalNorow=10;		
		try {
			log=new Log();	lbrDtlVoObj=new MvpBloodGroupTbl();			
			System.out.println("Step 1 : Bloodtype Type  select all block enter");
			log.logMessage("Step 1 : Bloodtype Type  select all block enter", "info", Bloodtypelistviewall.class);			
			...
			if(locvrCntflg!=null && (locvrCntflg.equalsIgnoreCase("yes") || locvrFlterflg.equalsIgnoreCase("yes"))){// for web 
				...
				{
					locvrEdu_cntQry="select count(bloodGroupId) from MvpBloodGroupTbl where status in(0,1)";
				}
				else
				{
					locvrEdu_cntQry="select count(bloodGroupId) from MvpBloodGroupTbl where status in(0,1) AND (" + "bloodGroupName like ('%" + loctocheNull+ "%')  " 
			                                + ")";
				}
				System.out.println("Step 2 : Count / Filter Count block enter SlQry : "+locvrEdu_cntQry);
				log.logMessage("Step 2 : Count / Filter Count block enter SlQry : "+locvrEdu_cntQry, "info", Bloodtypelistviewall.class);
				...
				countFilter=IdcardDaoObj.getTotalFilter(locvrEdu_cntQry);
				 ...
				 log.logMessage("Step 2 : Count / Filter Count not need.[Mobile Call]", "info", Bloodtypelistviewall.class);
			}
				...
				String locOrderby =" order by bloodGroupId desc";
			if(loctocheNull!=null && !loctocheNull.equalsIgnoreCase("null") && !loctocheNull.equalsIgnoreCase("")){
				  globalsearch = " AND (" + "status like ('%" + loctocheNull+ "%') or " 
			                                + "bloodGroupName like ('%" + loctocheNull + "%'))";
				  loc_slQry="from MvpBloodGroupTbl  where status in(0,1) " +globalsearch+" "+locOrderby;
			}else{
				loc_slQry="from MvpBloodGroupTbl  where status in(0,1)"+locOrderby;	
			}				
			...
			log.logMessage("Step 3 : Bloodtype Type  Details Query : "+loc_slQry, "info", Bloodtypelistviewall.class);
			...
			while ( locObjLbrlst_itr.hasNext() ) {
				...
				lbrDtlVoObj = (MvpBloodGroupTbl) locObjLbrlst_itr.next();								
				locInrJSONObj.put("bloodtype_id",lbrDtlVoObj.getBloodGroupId());	
				locInrJSONObj.put("bloodtype_name",lbrDtlVoObj.getBloodGroupName());
				locInrJSONObj.put("bloodtype_status",lbrDtlVoObj.getStatus());									
			...
			locFinalRTNObj.put("bloodtypedetails", locLBRJSONAryobj);			
			System.out.println("locFinalRTNObj : "+locFinalRTNObj);
			System.out.println("Step 6 : Select category detail block end.");
			log.logMessage("Step 4: Select category detail block end.", "info", Bloodtypelistviewall.class);
			return locFinalRTNObj;
		} catch (Exception e) {
			try{
			System.out.println("Exception in toCMPYSelectAll() : "+e);
			log.logMessage("Step -1 : category select all block Exception : "+e, "error", Bloodtypelistviewall.class);	
			locFinalRTNObj=new JSONObject();
			locFinalRTNObj.put("InitCount", count);
			locFinalRTNObj.put("countAfterFilter", countFilter);
			locFinalRTNObj.put("bloodtypedetails", "");
