...
public class DonationDelete extends ActionSupport {
	Log log=new Log();
	otpDao otp=new otpDaoService();
	private String ivrparams;
	DonationDao donationhbm=new DonationDaoServices();
	CommonUtils commjvm=new CommonUtils();

	public String execute(){

		System.out.println("************mobile Donation Delete services******************");
		...
		try{
			...
			if (ivIsJson) {
				...
				String Donation_id = (String) Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson, "Donation_id");
			...
			if(userMst!=null){
				...
				if(Donation_id!=null && Donation_id.length()>0){

				boolean Donationid=donationhbm.deleteDonationId(rid,Donation_id);
				locObjRspdataJson=new JSONObject();
				if(Donationid){
				serverResponse(servicecode,"00","R0248",mobiCommon.getMsg("R0248"),locObjRspdataJson);
				}else{
					locObjRspdataJson=new JSONObject();
			...
			locObjRspdataJson=new JSONObject();
			log.logMessage("Service code : "+servicecode+" Request values are not correct format", "info", DonationDelete.class);
			serverResponse(servicecode,"01","R0016",mobiCommon.getMsg("R0016"),locObjRspdataJson);
		}
		}catch(Exception ex){
			System.out.println("=======Donation Delete====Exception===="+ex);
			log.logMessage("Service code : ivrservicecode, Sorry, signUpMobile an unhandled error occurred", "info", DonationDelete.class);
			locObjRspdataJson=new JSONObject();
			serverResponse(servicecode,"01","R0002",mobiCommon.getMsg("R0002"),locObjRspdataJson);
		}
