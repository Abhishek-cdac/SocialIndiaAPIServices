...
public class DocConfigDaoservice implements DocConfigDao{
	...
	public boolean saveLaborInfo(LaborProfileTblVO pIntLabrobj) {
		...
		try {
			...
			System.out.println("Exception found in DocConfigDaoservice : "+e);
			log.logMessage("Exception : "+e, "error", DocConfigDaoservice.class);
			return false;
		} finally {
	...
	public int  toInsertdoctypetype(DoctypMasterTblVO doctypetypeTblObj) {
		...
		try {
			session = HibernateUtil.getSessionFactory().openSession();
			tx = session.beginTransaction();			
			session.save(doctypetypeTblObj);
			locidcardid=doctypetypeTblObj.getIvrBnDOC_TYP_ID();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
				tx.rollback();
			...
			log=new Log();
			System.out.println("Exception found in DocConfigDaoservice.class : "+e);
			log.logMessage("Exception : "+e, "error", DocConfigDaoservice.class);
			locidcardid=-1;
			return locidcardid;
		} finally {
	...
	public String toExistDocumenttypelistname(String lvrdoctypetypetitle) {
		// TODO Auto-generated method stub
		  String query = "SELECT CASE WHEN COUNT(*)=0 THEN 'NEW' ELSE "
		          + "'ALREADY EXISTS' END FROM DoctypMasterTblVO where ivrBnDOC_TYP_NAME='"
		          + lvrdoctypetypetitle + "'";
		  Log log=null;
			Session session = null;
			Transaction tx = null;
